...
class CandidateRowsRelationManager extends RelationManager
{
        ...
        return $schema
            ->components([
                ...
                TextEntry::make('conflict_summary')
                    ...
                    ->state(fn (CandidateScheduleRow $record): string => self::payloadSummary($record->conflict_payload))
                    ->placeholder('None')
                    ->columnSpanFull(),
                TextEntry::make('warning_summary')
                    ->label('Warnings / Review Notes')
                    ->state(fn (CandidateScheduleRow $record): string => self::payloadSummary($record->warning_payload))
                    ->placeholder('None')
                    ->columnSpanFull(),
        ...
        return $table
            ...
            ->columns([
                ...
                TextColumn::make('conflict_count')
                    ->label('Conflicts')
                    ->state(fn (CandidateScheduleRow $record): int => self::payloadCount($record->conflict_payload))
                    ->badge()
                    ->color(fn (int $state): string => $state > 0 ? 'danger' : 'success'),
                TextColumn::make('warning_count')
                    ->label('Warnings')
                    ->state(fn (CandidateScheduleRow $record): int => self::payloadCount($record->warning_payload))
                    ->badge()
    ...
    private static function reviseAction(): Action
    {
        return Action::make('reviseDraftRow')
            ...
            ->color('warning')
            ->visible(fn (CandidateScheduleRow $record): bool => self::canRevise($record))
            ...
            ->schema([
                Select::make('section_delivery_group_id')
                    ->label('Delivery Group')
                    ->options(fn (CandidateScheduleRow $record): array => SectionDeliveryGroup::query()
    ...
    private static function canRevise(CandidateScheduleRow $record): bool
    {
        $user = auth()->user();
