...
{
    public function execute(FeePlan $feePlan, User $actor, string $authorityReference): FeePlan
    {
        ...
        return DB::transaction(function () use ($feePlan, $actor, $authorityReference): FeePlan {
            ...
            if ($locked->state !== FeePlan::StateDraft || $locked->charges->isEmpty() || blank($authorityReference)) {
            ...
            $payload = [
                ...
                'charges' => $locked->charges->map->only(['sequence', 'code', 'label', 'amount'])->all(),
                'obligations' => $locked->obligations->map->only(['code', 'label', 'amount', 'required_for_enrollment'])->all(),
            ];
            $locked->update([
