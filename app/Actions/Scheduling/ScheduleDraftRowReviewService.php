...
class ScheduleDraftRowReviewService
{
     ...
     */
    public function revise(ScheduleDraftRow $draftRow, array $data, User $registrar): ScheduleGenerationRun
    {
        ...
        return DB::transaction(function () use ($draftRow, $prepared, $registrar): ScheduleGenerationRun {
            /** @var ScheduleDraftRow $lockedRow */
            $lockedRow = ScheduleDraftRow::query()
                ->with('generationRun')
                ->lockForUpdate()
            ...
            $solverResult = [
                'draft_rows' => $draftRows
                    ->map(fn (ScheduleDraftRow $row): array => $row->id === $lockedRow->id
            ...
            if ($reviewedRow instanceof ScheduleDraftRow) {
     ...
     * @return array<string, mixed>
     */
    private function existingRowPayload(ScheduleDraftRow $row): array
    {
        return [
            'section_id' => $row->section_id,
