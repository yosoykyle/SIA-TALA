...
**School-First Institutional Branding:** Institutional authority belongs to the school. All workspaces, surfaces, official documents, and notifications must lead with the official institutional identity: **Servitech Institute Asia Inc.** (or **Servitech Institute Asia**). System identity is strictly secondary (e.g., *Powered by TALA* or *Generated through TALA from authenticated records*). Bare "TALA Staff Workspace", standalone "TALA" brandings, or system-first presentations where institutional authority is exercised are prohibited.
...
**Administrative Holds Subsystem Boundary:** Administrative holds are strictly action-scoped restrictions defined by the `Hold` model and schema (`blocking_level`: `blocks_enrollment`, `blocks_cor_print`, `blocks_clearance`, `blocks_record_release`, `blocks_graduation_eligibility`, `blocks_reactivation`, `advisory_only`). Blanket, wildcard, or global holds that block all services or student account access are prohibited. Every hold records responsible office attribution (`financial` owned by Accounting; `academic_deficit` and `prerequisite` owned by Academic Head; all other types owned by Registrar), internal `reason` and `staff_only_reason`, student-facing information (`student_message` and `resolution_requirement` exposed via `studentFacingMessage()`), placing actor (`created_by`), optional expiry (`expires_at`), and an immutable resolution/waiver audit trail (`resolved_by`, `resolved_at`, `waived_by`, `waived_at`). Payment requirements and document clearances (`EnrollmentPaymentRequirementProjection`, `OfficialOutputPaymentClearance`) are separate source-specific ledger projections owned by Accounting, not administrative holds.
...
The database queue is appropriate while workload remains within the capacity of the primary database. Current implementation/configuration evidence checked on **August 19, 2026** resolves the Python solver's single handler-entry request budget to 300 seconds with a 15-second response reserve, the Laravel HTTP client and Gunicorn worker timeouts to 330 seconds, `ScheduleSolverDispatchJob::$timeout` and the Cloud Run service request ceiling to 360 seconds, and the database queue default `retry_after` to 420 seconds. The ordering is therefore **Python request budget 300 seconds < Laravel/Gunicorn 330 seconds < queue job 360 seconds < `retry_after` 420 seconds**. The separate Cloud Run 360-second provider ceiling equals the job timeout but does not replace the internal response reserve or Laravel client limit. Queue attempts and bounded backoff own retries; the HTTP client makes one request per attempt.

Any future timeout change must preserve a response reserve before the provider ceiling and keep the HTTP client below the queue job timeout and the job timeout below `retry_after`. Such a change requires coordinated application, worker, container, provider, and acceptance verification; one timeout must never be increased in isolation.
...
The [Human-Centered Operations design definition](TALA-Experience-and-Official-Output-Design-Definition.md) records the historically approved (August 2026) production-translation companion for these surfaces, while the selected replacement direction (Candidate #6: The Philippine Archival Collegiate Registry in seed `DESIGN.md`) guides active preimplementation design. Its prototype is historical evidence, not a frontend runtime or source dependency. Canonical architecture and UI authorities govern any conflict.
...
At 1024 CSS pixels and above, the authenticated shell uses Filament's native desktop-collapsible left navigation, a top bar, and one main region. Below that width, navigation becomes Filament's labelled modal drawer; TALA adds no role-specific bottom navigation or global search. The shell, navigation order, workspace switcher, Account Security entry, page-title/action hierarchy, breadcrumb/back-link rules, semantic landmarks, focus behavior, responsive transformations, visual tokens, and reusable component variants are governed by the UI Surface Blueprint. These presentation contracts reuse Filament/Livewire, building on the historically approved Human-Centered Operations direction while transitioning to the selected Candidate #6 replacement design; they do not introduce a SPA, a second authorization layer, a new public API, or a design-tool runtime dependency.
...
The scheduling service is an isolated private Python/OR-Tools CP-SAT adapter. Source identity `cloud-cp-sat-tala-timetable-v2-lexicographic-v1-deadline-v2` uses one handler-entry 300-second request budget and a 15-second response reserve. The accepted Issue #22 activation evidence dated **August 19, 2026** records private Cloud Run revision `tala-scheduler-solver-i22-3f461a46` in `asia-southeast1`, promoted to 100% traffic from immutable image digest `sha256:0f5b6ce1547d67308803c84427f4d2ca68e0017cd82e0dd7eb6558951de7c789`. Its verified profile was 8 vCPU, 16 GiB, eight solver workers, concurrency one, a 300-second internal request budget, a 360-second Cloud Run request ceiling, minimum zero instances, maximum two instances, and seed `20260718`. IAM remained private with no `allUsers`; only the dedicated scheduler invoker retained `roles/run.invoker`. Exactly one Canonical TALA Scheduling Dataset qualification returned a complete hard-valid `feasible` result for all 54 Scheduling Demands and passed independent Laravel validation, ingestion, publication, and role projections. This evidence is attributable only to that dated activation and may drift; it does not establish general capacity, optimality, production readiness, external-host credential custody, or unchanged live provider state.

**Evidence and Demonstration Boundary:** CP-SAT schedule generation is the principal capstone technical and algorithmic innovation. The accepted Issue #22 Cloud Run activation on August 19, 2026 (revision `tala-scheduler-solver-i22-3f461a46`) records an actual repository milestone, but the live external service state is unverified in this session. The local solver execution and client adapter orchestration are backed by implementation and automated test evidence (`tests/Unit/TAL65CloudRunSchedulingSolverClientTest.php`), but they are not verified live endpoints or demonstrated live services. Capstone demonstration and local operational validation rely on this test-backed implementation pathway.

The product boundary remains an immutable whole-term source snapshot, a typed solver outcome, independent Laravel validation, human review, and Registrar publication. The current journey uses one Canonical TALA Scheduling Dataset rather than active MIN/MIDDLE/MAX selectors. Historical 1/1, 2/4, 4/8, and earlier 8/8 or 8/16 benchmark results remain compact scaling and failure evidence only; their executable reproduction machinery is not part of the product runtime.
...
**Provider-Neutral Accounting and Implementation-Evidence Boundary:** Core accounting, fee plans, assessments, obligations, and payment verification are strictly provider-neutral institutional capabilities. TALA supports manual payment verification (bank, cash/teller receipts) independently of third-party gateways. The PayMongo webhook and hosted checkout integration is backed by implementation and automated test evidence (`tests/Feature/Finance/ExactDuePayMongoJourneyTest.php`, `tests/Feature/TAL95BPayMongoWebhookPipelineTest.php`), but live sandbox and production gateway endpoints are unverified in this session. Live merchant activation remains strictly owner-gated.
...
| Scheduling service | Private `tala-scheduler-solver` in `asia-southeast1`; Python 3.12 container; 8 vCPU; 16 GiB; eight solver workers; concurrency one; 360-second Cloud Run request ceiling; minimum zero and maximum two instances; seed `20260718`. Source identity `cloud-cp-sat-tala-timetable-v2-lexicographic-v1-deadline-v2` uses a 300-second request budget and 15-second reserve. | Issue #22 activation evidence dated August 19, 2026 records revision `tala-scheduler-solver-i22-3f461a46` at 100% traffic from immutable image digest `sha256:0f5b6ce1547d67308803c84427f4d2ca68e0017cd82e0dd7eb6558951de7c789`, with private IAM and the accepted profile. The evidence proves that attributable activation only; it may drift and does not establish general capacity, optimality, production readiness, credential custody, or unchanged live state. Laravel independently exposes configured identity, last observed result identity, and active deployment evidence without conflating them. |
...
For the scheduling row, a **solver worker** is one CP-SAT search thread inside a request, while **concurrency one** means one HTTP solver request at a time per service instance. The settings are not interchangeable. Deployment, traffic, provider compatibility, and observed Cloud behavior require separate operational evidence even when the source implementation and local acceptance are complete.
...
The primary host receives only a repository-scoped credential. The client-side encryption/recovery key is not stored solely on that host, with its provider, or in the selected backup provider; controlled institutional recovery copies are required. No credential, personal-data payload, provider control, shell command, media command, or restore action appears in TALA. The existing System Health surface may show only safe locally recorded job outcome, latest verified generation/as-of time, overdue state, and next action; provider and physical-media state use plain not-recently-checked language and never appear healthy without separately imported approved evidence. Exact custody and evidence boundaries remain behind technical details.

The implemented application boundary records external backup and isolated-restore results through `tala:operations:record-backup-evidence` and `tala:operations:record-restore-evidence`. Both commands accept one strictly allowlisted versioned JSON object, reuse `operational_events`, and perform no backup, restore, provider, media, or shell operation. Identical references are idempotent, conflicting reuse is rejected, corrections append and reference the superseded event, and event time—not ingestion order—determines the current projection. `SYS-003` reads this evidence alongside bounded local application, database, private-storage, queue, mail, PayMongo, and solver evidence; external facts remain `Unknown`. `SYS-004` projects allowlisted institutional changes, system events, and output-access evidence directly from their authoritative stores and provides the fixed privacy/retention boundary without a parallel audit store or generic report export.
...
- Show only locally recorded evidence in System Health; use plain language for provider and physical-backup facts that were not recently checked, with source/as-of/boundary under technical details. Keep Governance & Audit read-only and say **Automatic record disposal is not available in TALA. Follow the institution's approved privacy and records procedure.** Preserve the exact no-automatic-disposal boundary in technical detail without claiming to assess external compliance.
...
- [TALA Experience and Official-Output Design Definition](./TALA-Experience-and-Official-Output-Design-Definition.md) — historically approved Human-Centered Operations rationale and production-stack translation; subordinate to canonical product, UI, and architecture authorities.
