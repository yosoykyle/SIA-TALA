...
| Accounts and payment evidence | Derives one continuous Term Account, current-due position, bounded Clinic 4/5 projections, verified postings, and non-tax account outputs from approved sources | Accounting publishes Fee Plans, checks external/manual evidence, resolves exceptions and corrections, and issues any required tax document outside TALA |
...
- Clinic 6 `EnrollmentPaymentRequirementProjection`, whose clearance means the amount currently required is covered rather than requiring a universal zero balance.
...
Faculty calculates period grades and raw scores outside TALA. Clinic 5 stores no Preliminary/Midterm/Final values, formula engine, attendance gradebook, or released `P`. A complete roster moves through `Draft`, `Submitted`, `Returned`, and `Released`; only Registrar release creates immutable `OfficialGradeEvent` records. `INC`, authorized correction, progress consequence, lifecycle change, approved credit/equivalency, honor, and conferral remain typed recorded outcomes rather than a generic policy or approval engine.

Clinic 5 derives `StudentGwaProjection`, `CurriculumEvaluation`, academic-progress recommendation, `CompletionReadiness`, and immutable transcript snapshots. It exposes only released `OfficialCourseResultProjection`, confirmed `AcademicEnrollmentEffect`, approved credit/equivalency, and effective lifecycle facts to Clinic 4. An affected active Registration Case enters Registrar review after correction; no academic-record change silently modifies a registration.
...
- An applied adjustment or Course Drop synchronizes official placement, roster, schedule, account-review projection, and a new COR version. Timetable impact is resolved before Clinic 3 finalizes an affected class cancellation.
...
- INC resolution/lapse and authorized grade correction append superseding result events. They recalculate affected GWA, curriculum-evaluation, progress, and completion projections while retaining history; affected active registrations are flagged for review rather than silently changed.
...
- Keep final-grade calculation outside TALA and the official academic record inside it. Clinic 5 accepts one controlled final result per official roster row, releases complete rosters through Registrar, preserves immutable result/lifecycle/transcript history, and derives GWA, curriculum evaluation, progress, and completion without a gradebook, policy DSL, what-if audit, or transcript-template editor.
- Keep Accounting authority outside a general finance platform. Clinic 6 uses one versioned Program-and-Term Fee Plan, one continuous same-human-subject/RegistrationCase Term Account, exact current-due payment evidence, append-only corrections, and bounded Clinic 4/5 projections without Fee Rule precedence, a silent percentage fallback, prior-debt allocation, Billing Slip/OR mapping, global holds, or a report hub. `Person` is a documentation label for identity continuity, not a universal master record.
...
The architecture has passed the final cross-module authority review. It may now inform separately planned journey-complete implementation tasks; it does not itself authorize code, schema, dependency, deployment, or external-system changes.

---

## 18. Sources and References

Architecture-wide sources were checked on **July 14, 2026**; Clinic 5 academic-record sources were checked on **August 5, 2026**; and Clinic 6 policy, tax-document, privacy, and Hostinger sources were checked on **August 6, 2026**, unless a separate publication or bulletin date is stated.
...
- [PUP Student Handbook](https://www.pup.edu.ph/studentservices/files/ThePUPStudentHandbook2014.pdf) — Philippine reference for INC completion and the transparent academic-progress profile; not represented as Servitech or CHED-wide policy.
