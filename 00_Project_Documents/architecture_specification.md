...
| Official outputs | Generates COR, schedules, unofficial academic record, TALA Standard TOR snapshots, non-tax Account Statement/SOA and Payment Acknowledgment, and approved contextual exports from authoritative records | The owning office controls issuance/correction; physical signature, seal, delivery, CAV, and any external registered invoice remain external |
...
For one Individually Advised continuing-Student case, Clinic 4 may record an append-only `AuthorizedPrerequisiteException` while the exact prerequisite result remains unreleased. The authority binds Student, Registration Case, source/consuming Terms, prerequisite registration/result, dependent Course/Class Offering, office/reference/date, reason, exact permission, effective period, and expiry. It is consumed only by the named proposal, placement, confirmation, and finalization checks. It creates no grade, `P`, satisfaction, credit, units, GWA, completion effect, permanent learner status, generic override API, or policy engine.
...
- A prerequisite exception is append-only and exact-scope. Finalization locks its authority/source/expiry versions with the proposal and placement; a released result or elapsed expiry before commit invalidates the transaction rather than silently extending permission.
...
- INC completion and authorized grade correction append superseding result events. A deadline amendment appends its own authority/reason history; deadline passage only derives `CompletionOverdue` and never writes a grade. Transactions revalidate the current unresolved event/version. Every initial release or released successor recalculates affected averages, curriculum evaluation, `AcademicEnrollmentEffect`, completion, prerequisite-exception, and active-case projections while retaining history. Duplicate event handling returns the same review; registrations are never silently changed.
...
| Backup export to Cloudflare R2 | Transaction-consistent source, client-side restic encryption, TLS, bucket-scoped credential, private bucket, retained generations, locally recorded integrity/outcome evidence |
| R2 to recovery workstation and ORICO | Named Infrastructure Custodian, separate institutional recovery-password custody, verified repository copy, encrypted independent media, safe unmount, disconnection, and separate physical custody |
...
The private Cloud Run service and its dedicated invoker authorization are current configuration evidence; they do not settle external-host credential custody. Before production acceptance, the institution and its authorized infrastructure custodian must name the accountable owners and approve and evidence the mechanisms above. The R2/restic recovery provider and role-level responsibility are planning selections, but no named person, security-log retention duration, configured alert channel, implemented secret-custody mechanism, or achieved control is inferred here. PRD 01's bounded security evidence and PRD 06's `Not checked by TALA` presentation remain authoritative in the product UI.
...
The selected initial production sender is one dedicated automated mailbox on the institution-owned domain, such as `notifications@the-approved-domain`, using Hostinger Business Email Starter. Laravel remains configured through the standard provider-neutral SMTP settings: authenticated `smtp.hostinger.com` transport with TLS/STARTTLS on port 587, or the provider-documented SSL port 465 alternative, and credentials injected only through the protected deployment environment. An institution-owned human address may be configured as `Reply-To`, but this decision does not create or assume a Staff mailbox suite.

Production mail readiness requires the domain's MX, SPF, DKIM, and DMARC records to be configured and verified, plus a recorded authenticated delivery test. Hostinger Business Email Starter currently publishes a per-mailbox limit of 1,000 inbound and 1,000 outbound messages per rolling 24 hours; provider limits may change and must be rechecked in hPanel before production acceptance. Gmail is limited to development or manual-delivery-test evidence and is never the selected production sender. Automated tests continue to use Laravel's array or log mail transport and must not depend on a live provider.

Hostinger Email is the lean starting provider because it centralizes the bounded SIS host, DNS, domain, and one automated mailbox under one institution-owned account. It is not universally superior and creates provider-concentration risk. If measured peak volume, delivery evidence, provider limits, or operational needs exceed the selected service, the institution may move to another authenticated SMTP provider; Brevo is one documented fallback candidate, not the baseline selection or a priced commitment. Because Laravel keeps the SMTP contract provider-neutral and the application keeps notifications queued, idempotent, and failure-safe, that move is a deployment-configuration change rather than an application rewrite.
...
The 8 GB KVM 2 VPS co-locates Nginx, PHP-FPM, Laravel, MySQL, and an initial queue worker. Compared with the provider's KVM 1 tier, it doubles the published vCPU, memory, NVMe storage, and bandwidth so enrollment, grade release, queue processing, integration callbacks, backup activity, and near-term population growth begin with proportionate headroom rather than the smallest available tier. That selection does not guarantee unlimited scale or remove the need to monitor concurrent users, database size, document-upload volume, queue depth, response time, and recovery activity. Sustained memory pressure, swapping, disk pressure, slow database queries, queue delay, or missed response/recovery objectives must trigger resizing or separation of the database and workers. Node.js and Python are not required on the web host when frontend assets are prebuilt and the solver remains externally deployed.
...
flowchart LR
    ...
    subgraph HostingerAccount["Institution-owned Hostinger account / hPanel"]
        DNS["Institution-owned custom domain<br/>Hostinger DNS"]
        MAIL["Business Email Starter<br/>one automated sender mailbox"]
        VPS["KVM 2 subscription"]
    end

    subgraph HostZone["Initial Hostinger KVM 2 failure domain"]
    ...
    subgraph Providers["External provider trust zones"]
        ...
        PM["PayMongo"]
        SMTP["Configuration-level SMTP fallback<br/>only when evidence requires"]
    end

    subgraph BackupZone["Off-server / offline backup trust zones"]
        OS["Private Cloudflare R2 Standard bucket<br/>encrypted restic repository"]
    ...
    B --> E
    E --> OS
    OS -->|weekly verified repository copy| IWC
    IWC -->|alternating offline rotation| OH
```

A single Hostinger KVM 2 VPS is a lean self-managed starting topology with deliberate initial headroom, not a highly available or indefinitely scalable one. Nginx terminates web traffic, PHP-FPM runs Laravel, MySQL holds authoritative data, and a supervised queue worker processes asynchronous work. The same institution-owned Hostinger account initially owns the VPS subscription, custom domain and DNS, and one automated-sender mailbox. This centralization reduces handoff and billing surfaces for the bounded SIS, but it concentrates account, provider, DNS, mail, application, and database risk. Portable DNS records, provider-neutral SMTP configuration, documented deployment procedures, independent backups, and measured resource monitoring provide escape paths; they do not make Hostinger universally superior, eliminate migration work, or replace evidence-based scaling. Hostinger's published weekly VPS backup and manual snapshot are supplemental recovery layers; weekly recovery points alone cannot satisfy TALA's six-hour RPO target.

The Hostinger owner account must be registered to and controlled by the institution, protected by MFA, and retained through handover. Developers receive delegated access rather than shared owner credentials. The institution owns renewal payment methods, a renewal calendar and accountable renewal contact for the VPS/domain/mailbox, and recovery access. Application, database, SMTP, and provider credentials remain only in the protected deployment environment. Independent off-server and offline backups remain outside the Hostinger failure domain.

Philippine privacy authority requires a proportionate continuity process covering personal-data backup, restoration, remedial time, and periodic review/testing. It does not prescribe a numeric recovery objective. The following design is therefore a **project-authorized prospective production baseline**, not a law- or Servitech-mandated policy, SLA, current control, or achieved recovery claim:
...
- Hostinger's included weekly full-VPS backups and one controlled pre-change snapshot remain supplemental infrastructure-recovery points. The optional paid daily plan is not selected initially because it remains in the Hostinger provider/account boundary and cannot meet a six-hour recovery point.
- At least every six hours, one non-overlapping operating-system job creates a transaction-consistent export of authoritative application data, required private documents and reproducible official-output source files, plus a manifest containing backup time, application revision, migration state, and integrity evidence. Authoritative audit, payment, webhook, email-idempotency, and correction evidence are protected. Rebuildable caches and sessions, temporary exports, replaceable build files, Git-held source, and recovery secrets are excluded or rebuilt.
- The selected mechanism is a client-side encrypted restic-compatible S3 repository in a private institution-owned Cloudflare R2 Standard bucket. R2 is outside the Hostinger host and owner-account failure boundary; its provider-side encryption is supplemental. The bucket uses the best-effort `apac` location hint, not a Philippine residency claim.
- The off-provider repository keeps every six-hour snapshot for seven days, one daily snapshot for 30 days, and one monthly snapshot for 12 months. Those generations are disaster-recovery retention, not institutional record archival retention, and do not extend a lawful live-record retention period.
- A failed scheduled run retries once after 30 minutes. A failed retry alerts the named Infrastructure Custodian and repeats the alert daily until recovery. The locally recorded protection state becomes overdue when the latest verified off-provider generation exceeds six hours. Repository integrity is checked weekly.
- Once per week, the Infrastructure Custodian uses an authorized recovery workstation to copy the latest verified off-provider repository to alternating independently encrypted ORICO Drive A and Drive B media. Each drive contains its own recoverable repository; the drives are not combined as RAID. At least one verified drive remains safely unmounted, disconnected, and separately stored.
- An isolated end-to-end restore is recorded quarterly, alternating R2, Drive A, and Drive B so every recovery source is proved within one year. The test records selected generation, measured data loss, measured restoration duration, integrity result, critical-journey result, owner, and remediation.
...
The selected R2/restic direction is not complete merely because an object-storage account exists. Production still requires an implemented consistent export, monitored scheduling, current verified generations, scoped credentials, retained keys, and successful restoration. The institution must approve the R2 processor terms, international-transfer safeguards, billing owner, best-effort data-location treatment, and current checkout price. The institution remains the Personal Information Controller and owns the Hostinger, R2, domain, billing, and recovery accounts. Its privacy/DPO authority approves the processor, transfer, retention, removable-media, and lawful-disposal rules. A named **Infrastructure Custodian** operates monitoring, rotation, recovery, and escalation. Developers may configure and document the mechanism during deployment but must not retain sole owner credentials after handover.

The VPS receives only a bucket-scoped R2 credential. The restic recovery password is not stored solely on the VPS, in Hostinger, or in R2; two separately controlled institutional recovery copies are required. No credential, personal-data payload, provider control, shell command, media command, or restore action appears in TALA. The existing System Health surface may show only safe locally recorded job outcome, latest verified generation/as-of time, overdue state, and next action; provider and physical-media state remain `Not checked by TALA` without separately imported approved evidence.

Recovery first provisions or validates clean infrastructure and restores the newest valid R2 generation into isolation. A Hostinger provider backup may accelerate infrastructure restoration, but newer independent authoritative data must then be reconciled. If Hostinger or R2 access/integrity is unavailable, recovery falls back to the newest valid ORICO repository. Before authorized cutover, the custodian verifies the manifest, database, private files, authentication, and critical journeys; invalidates restored sessions; clears rebuildable caches; reconciles queued/integration work idempotently; and reapplies any externally recorded lawful disposition made after the selected generation.

The **RPO target is six hours of authoritative database/private-file changes**. The **RTO target is eight elapsed hours** after recovery is formally declared and the required accounts, keys, media, infrastructure, and personnel are available, ending when core authenticated SIS service with verified current authoritative data is restored. External integrations may remain explicitly degraded. These are project-authorized engineering targets to test and do not become achieved until operational restore evidence proves them.

As of **August 11, 2026**, the provider/mechanism and responsibility design is selected, but no tracked evidence proves R2 procurement, production restic automation, alert delivery, named-person assignment, completed ORICO rotation, current retained generations, or a successful end-to-end restore. Those remain owner-bound production gates and must never be presented as operating controls before evidence exists.

The client-supplied enclosure is the **ORICO 9548U3** four-bay 3.5-inch hard-drive enclosure. Its host connection is USB 3.0 Type-B with a maximum interface rate of 5 Gbps; internally it uses a SATA 3.0 bridging scheme for compatible SATA HDDs. ORICO specifies push-pull installation, Windows/macOS/Linux compatibility, a built-in 150 W power supply for the four-bay version, and a maximum supported capacity of 64 TB. The prospective starting deployment uses only two 4 TB drives. That size is conservative planned headroom and a priced procurement scenario, not a legal requirement or capacity derived from 47 people. Each drive must hold the complete retained encrypted repository with at least 25% free space at procurement and review time. The remaining two bays are expansion positions and do not authorize unmeasured capacity purchases or an undocumented RAID conversion. Sources: [ORICO 9548U3 product specification](https://www.orico.cc/index/product/detail/2056.html?mtpl=1) and [ORICO 95U3 series manual](https://orico.cc/storage/attachments/20250415/f593f8ef0bbef6c65c90e7a9b049dc36.pdf).
...
| Hostinger Email or configured SMTP provider | The underlying institutional transaction remains valid. Delivery is retried or recorded as failed for operational follow-up. If measured volume, delivery, limit, or operational evidence requires a provider change, switch through protected SMTP configuration without rewriting the notification domain behavior. |
| Hostinger DNS or domain control | New name resolution or renewal failure can make web and mail entry unavailable. Keep institutional ownership, MFA, delegated access, renewal controls, current DNS records, and a documented transfer/recovery procedure; never treat email as the source of truth. |
| Hostinger VPS or MySQL | Web workspaces are unavailable. Recovery validates clean infrastructure and restores the newest valid R2 generation in isolation. A provider backup may accelerate infrastructure recovery but is supplemental and must be reconciled with newer independent data. ORICO is the fallback when provider or R2 access/integrity is unavailable. Cutover requires manifest, data, private-file, authentication, critical-journey, queue/integration, session/cache, and post-backup-disposition reconciliation under the six-hour RPO/eight-hour RTO test procedure. |
| Scheduled backup job | Prevent an overlapping run. Retry once after 30 minutes; if the retry fails, alert the Infrastructure Custodian and repeat daily until a valid generation exists. Mark the local protection result overdue after six hours without a verified off-provider generation; the application may continue, but no healthy recovery claim is allowed. |
| Cloudflare R2 or scoped credential | The application may continue temporarily, but the primary off-provider recovery point is impaired. Do not redirect backups into Hostinger or infer that weekly/daily provider copies meet the six-hour target. Restore scoped access or an approved equivalent off-provider destination, create and verify a catch-up generation, then clear the overdue state. |
| Restic recovery password or institutional recovery copy | Recovery stops without exposing or guessing the secret. Escalate to the separate institutional custodian, use the second controlled recovery copy, and rotate credentials only through the recovery runbook. Loss of every copy is an unrecoverable-generation failure, not an application reset path. |
| ORICO enclosure or one rotation HDD | The live system and independent R2 copy remain authoritative and available. Quarantine the failed component, keep the other verified rotation drive offline, procure a compatible replacement, recreate the encrypted copy through the authorized recovery workstation, verify it, and record a restore test before returning to weekly alternating rotation. |
...
This estimate preserves the original **July 14, 2026** planning basis and uses the cost/provider evidence refreshed on **August 11, 2026, Philippine Time**, with the selected Hostinger VPS plan rechecked on **August 13, 2026, Philippine Time**. USD prices are converted at **US$1 = ₱61.55**, the reference rate in the independently retrieved Bangko Sentral ng Pilipinas bulletin used for this revision, dated July 3, 2026. The exchange rate is an estimate input, not a guaranteed bank or card settlement rate.
...
The baseline assumes one prospective production institution; one self-managed Hostinger KVM 2 VPS; the provider's included weekly backup as a supplemental copy; an institution-owned Cloudflare R2 Standard bucket containing client-side encrypted restic six-hour backup generations; the client-owned four-bay ORICO 9548U3 with two independently encrypted 4 TB CMR NAS HDDs used in weekly alternating rotation; one client-approved institution-owned `.com` domain registered and DNS-managed through Hostinger when no suitable existing domain is available; one Hostinger Business Email Starter automated-sender mailbox; and database-backed queue/cache. Hostinger paid daily backups are not selected initially. If the institution already owns an appropriate domain, a bounded subdomain is preferred and the existing domain's renewal/operations cost remains part of the institution's wider portfolio rather than a duplicate TALA purchase. `.edu.ph` is an optional later institutional-identity upgrade or alias only after PHNET eligibility evidence and current authorization exist; it is not in the baseline and does not prove accreditation. Cloud Run and R2 remain usage-dependent. Taxes, payment-processor fees, provider upgrades or SMTP fallback, overages, foreign-exchange spreads, shipping, retrieval, and future replacement or expansion media remain excluded until attributable evidence exists.

Cost statements use these labels consistently:

| Label | Meaning in this estimate |
| --- | --- |
| **Fixed known planning floor** | Dated host/domain/one-mailbox arithmetic only; not complete production TCO |
| **Promotional / renewal** | A dated paid-upfront entry offer versus the selected renewal-equivalent basis |
| **Usage-dependent** | Cloud Run, email beyond the selected provider's limits, storage volume, retrieval, networking, logging, and overages |
| **Transaction-dependent** | PayMongo fees driven by successful channel volume and actual merchant terms |
| **Unquoted** | Operations/support and other items with no approved quotation; R2 itself uses its published usage formula until an invoice exists |
...
| --- | ---: | ---: | ---: | --- |
| Hostinger KVM 2 renewal basis | ₱819/month equivalent when renewing for two years; provider page showed ₱529/month promotional entry | ₱819.00 | ₱9,828.00 | Hosts Nginx, PHP-FPM, Laravel, MySQL, and the initial queue worker with proportionate initial headroom; renewal basis avoids treating a promotion as the steady-state price |
...
| Cloudflare R2 Standard encrypted backup repository | First 10 GB-month free; then US$0.015 per additional GB-month, with published monthly operation allowances and free direct egress | **₱0.00–₱83.09 illustration** | **₱0.00–₱997.11 illustration** | Required off-provider six-hour generations; range shows 10–100 retained GB using the dated BSP reference, before tax/FX and actual usage |
| Institution-owned `.com` domain renewal basis | ₱1,019/year; published example excludes VAT | ₱84.92 monthly equivalent | ₱1,019.00 | Selected custom-domain planning basis only when no appropriate institution-owned domain already exists |
| Hostinger Business Email Starter renewal basis | ₱49/month equivalent for one mailbox on a 48-month prepaid renewal term | ₱49.00 | ₱588.00 | One dedicated automated sender; no Staff mailbox suite is assumed |
| **Known fixed baseline before object storage** |  | **₱952.92** | **₱11,435.00** | Honest known host-domain-mailbox floor; not the complete production cost |

Hostinger displays monthly equivalents for prepaid VPS and Business Email terms. The `.com` example is a 12-month registration/renewal price and expressly excludes VAT; the email promotion and renewal equivalents use 48-month prepaid terms. Promotional entry prices are not used for the steady-state total. Cloudflare R2 bills measured storage and operations rather than a fixed TALA plan: at the dated exchange rate, 25 retained GB is about ₱13.85/month, 50 GB about ₱36.93/month, and 100 GB about ₱83.09/month before tax/FX. Request usage is expected to remain within the published free allowances at this bounded cadence but must be measured. The known subtotal remains a planning floor, and actual checkout/invoice and consumption control the final price.
...
The physical-backup estimate below is a Philippine procurement snapshot checked on **Tuesday, July 28, 2026, Philippine Time**. The four-bay ORICO 9548U3 is already owned by the client and therefore adds no new enclosure cash requirement, but its current Philippine reference price is retained as replacement-value and total-system-cost evidence. The selected starting capacity is two independent 4 TB drives in Bays 1 and 2: one Drive A copy and one Drive B copy, not a combined 8 TB array. This is conservative planned headroom and a priced procurement scenario; neither Philippine authority nor the 47-person synthetic profile requires 4 TB. Purchase remains gated on final cash approval and confirmation that each drive holds the complete retained encrypted repository with at least 25% free space. Bays 3 and 4 remain empty until measured growth justifies another matched pair or an approved additional-generation design.
...
| --- | ---: | ---: | --- |
| Hostinger renewal baseline with R2 range | ₱952.92 + measured R2, illustrated at ₱0.00–₱83.09 | ₱11,435.00 + measured R2, illustrated at ₱0.00–₱997.11 | Selected steady-state KVM 2, new `.com`, one mailbox, and required R2; 10–100 retained GB illustrates storage only, before tax/FX |
| Hostinger promotional illustration with R2 | ₱598.75 + measured R2 | ₱7,185.00 + measured R2 | ₱529/month VPS entry illustration + ₱609 first-year `.com` example + ₱19/month-equivalent mailbox promotion; provider terms are prepaid, periods differ, VAT/tax and checkout price remain excluded, and renewal controls steady-state planning |
| Existing institution-owned domain/subdomain | ₱868.00 + existing domain portfolio cost + measured R2 | ₱10,416.00 + existing domain portfolio cost + measured R2 | Avoids a duplicate domain purchase; arithmetic includes KVM 2 and one mailbox while the institution retains its existing domain renewal and DNS obligations |
| Optional Hostinger VPS daily backups | **Not selected; official help depicts US$6/month (about ₱369.30)** | **Not selected; about ₱4,431.60 before tax/FX** | Same-provider convenience retaining two recent daily and two weekly points; fixed schedule and whole-VPS overwrite do not meet or replace the independent six-hour baseline; hPanel checkout controls |
| Alternate SMTP provider fallback | **Not selected** | **Quotation required if triggered** | Brevo or another authenticated SMTP provider is considered only when measured volume, delivery, limits, or operational evidence exceeds Hostinger Email; no fallback price is in the baseline |
...
The encrypted R2 repository is mandatory in every prospective production scenario because Hostinger's weekly or optional daily provider backups cannot meet the independent six-hour target. A larger VPS, separated workers, or managed/high-availability database must be budgeted only when concurrent workload, maintenance, or tested recovery evidence justifies them.
...
| Hostinger Business Email Starter | One mailbox; published limit of 1,000 inbound and 1,000 outbound messages per rolling 24 hours, subject to provider change | Renewal-equivalent fixed price is in the baseline; measure peak volume and delivery, recheck hPanel limits, and quote an upgrade or provider-neutral SMTP fallback only when evidence requires it |
| Brevo or another SMTP fallback | No plan or price selected | Configuration-level fallback only; obtain a current quotation and verify limits, deliverability, DNS, tax, and operational fit before any selection |
...
| Cloudflare R2 Standard | First 10 GB-month free; US$0.015/GB-month thereafter; published monthly operation allowances; no direct-egress charge | Required selected provider; apply actual retained GB-month and operation usage, tax, and settlement FX. Procurement still requires institution-owned billing, processor/transfer approval, and current checkout verification |
| Hostinger VPS daily backups | Official help depicts US$6/month and retains two recent daily plus two weekly points; hPanel checkout controls | Optional and not selected initially; quote only if the institution wants same-provider restore convenience in addition to, never instead of, R2 |

Variable-price sources: [Google Cloud Run](https://cloud.google.com/run/pricing), [Cloudflare R2 pricing](https://developers.cloudflare.com/r2/pricing/), [Hostinger VPS backups](https://www.hostinger.com/support/1583232-how-to-back-up-or-restore-a-vps-at-hostinger/), [Hostinger daily backups](https://www.hostinger.com/support/1665153-how-to-activate-daily-backups-in-hostinger/), [Hostinger Email limits](https://www.hostinger.com/support/4625828-parameters-and-limits-of-hostinger-email/), and [PayMongo](https://www.paymongo.com/pricing). Actual R2 use, optional daily-backup checkout, and alternate-SMTP costs remain procurement-time values rather than fixed promises.
...
| --- | ---: | ---: | ---: | --- |
| **TALA known planning floor before object storage** | Per-institution host/domain/one-mailbox floor; not normalized by learner count | **₱952.92 + unquoted and usage-dependent costs** | **₱11,435.00 + unquoted and usage-dependent costs** | Not complete production TCO; required object storage, operations/support, usage, transactions, tax, and risk remain unresolved |
...
| --- | --- |
| Single Hostinger VPS and co-located MySQL form one failure domain | Selected prospective controls: monitored six-hour encrypted restic generations in institution-owned R2, supplemental Hostinger backup/snapshot, weekly alternating ORICO copies, quarterly isolated restore testing, and explicit scale/separation triggers. Procurement, named-person assignment, automation, current generations, alert delivery, media rotation, and achieved restore evidence remain unproved |
| R2 may process Philippine personal data outside a guaranteed Philippine location | Require institution-owned billing, privacy/DPO approval, processor and international-transfer safeguards, scoped credentials, client-side encryption, a best-effort `apac` hint labelled as non-residency, and a current provider/contract review before production use |
| Backup credentials, media, or retention can fail independently of application availability | Keep two separately controlled repository-password copies, weekly repository checks, non-overlapping jobs with one 30-minute retry and overdue alerting, two independently recoverable ORICO drives, quarterly source rotation, and measured restore evidence |
| VPS, DNS, domain, and automated mailbox start in one Hostinger account | Institution ownership, MFA, delegated developer access, renewal calendar/contact, documented DNS/deployment records, provider-neutral SMTP, and independent backups provide controlled handover and portable escape paths; centralization remains a concentration risk |
...
- Keep curriculum evaluation distinct from exact-Term registration. Clinic 4 stores versioned proposed course registrations inside one Registration Case, uses five accountable checkpoints, and may record one exact append-only prerequisite exception without creating a grade, satisfaction result, standalone Study Plan, policy-driving Regular/Irregular status, generic override engine, or global financial hold.
...
- Select an institution-owned Hostinger account as the lean initial production control surface: KVM 2 for the self-managed Laravel/MySQL/initial-queue topology, Hostinger DNS for a client-approved institution-owned `.com` domain when no suitable existing domain exists, and Business Email Starter for one dedicated automated-sender mailbox. KVM 2 supplies proportionate initial headroom for enrollment, grading, queues, integrations, backup activity, and near-term population growth, but it does not guarantee unlimited scale or remove measured upgrade triggers. Prefer a bounded subdomain of an appropriate existing institution-owned domain over a duplicate purchase. This centralization is a bounded starting choice, not a claim that Hostinger is universally superior.
...
- Keep Laravel SMTP provider-neutral. Use authenticated `smtp.hostinger.com` with TLS/STARTTLS port 587 or documented SSL port 465 for the starting mailbox; require MX, SPF, DKIM, DMARC, and delivery evidence; keep Gmail development/manual-test only and automated tests on array/log transports; treat Brevo only as a configuration-level fallback if measured volume, delivery, provider limits, or operational evidence requires it.
- Keep the Hostinger account, domain, mailbox, MFA, renewal ownership/calendar, and recovery access institution-owned. Use delegated developer access rather than shared owner credentials, keep secrets only in the deployment environment, and retain independent backups outside the Hostinger failure domain.
- Treat the six-hour RPO and eight-hour RTO as project-authorized prospective engineering targets, not law, Servitech policy, SLA, current capability, or achieved result. Use monitored encrypted restic generations in an institution-owned Cloudflare R2 Standard bucket every six hours; retain all generations for seven days, one daily for 30 days, and one monthly for 12 months; retry once after 30 minutes; alert and mark overdue after target breach; and prove recovery quarterly.
- Keep Hostinger weekly backups and controlled snapshots supplemental; do not purchase the optional daily-backup add-on initially or treat it as an independent six-hour layer.
...
- [UP November 2020 memorandum](https://publicservice.up.edu.ph/wp-content/uploads/2020/11/OVPAA-MEMO-2020-139_15-November-2020.pdf) — temporary exceptional-policy comparison for a pending prerequisite result, not a permanent Philippine or Servitech rule.
...
- Cloudflare: [R2 pricing](https://developers.cloudflare.com/r2/pricing/), [data location](https://developers.cloudflare.com/r2/reference/data-location/), and [data security](https://developers.cloudflare.com/r2/reference/data-security/) — Standard storage and operation formula, free allowances/direct egress treatment, best-effort `apac` location hint rather than residency guarantee, and provider-side encryption; checked August 11, 2026. Institutional processor/transfer approval and client-side restic encryption remain required.
...
- Brevo: [plan and email-limit documentation](https://help.brevo.com/hc/en-us/articles/208589409-About-Brevo-s-pricing-plans) — retained only as a provider-neutral SMTP fallback reference; no Brevo plan or price is selected for the production baseline, and any future choice requires a fresh quotation and delivery/limit review.
