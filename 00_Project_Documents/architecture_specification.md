...
The [Human-Centered Operations design definition](TALA-Experience-and-Official-Output-Design-Definition.md) records the historically approved (August 2026) production-translation companion for these surfaces. The September 25, 2026 user correction retains and refines the existing blue-led identity in seed `DESIGN.md` instead of adopting Candidate #6's replacement palette. Its prototype is comparison evidence, not a frontend runtime or source dependency. Canonical architecture and UI authorities govern any conflict.
...
**Evidence and Demonstration Boundary:** CP-SAT schedule generation is the principal capstone technical and algorithmic innovation. The accepted Issue #22 Cloud Run activation on August 19, 2026 (revision `tala-scheduler-solver-i22-3f461a46`) records an actual repository milestone, but its current live external state is unverified here. Issue #53 records a real local Python CP-SAT solve through independent Laravel validation, Registrar publication, and schedule projection on guarded `test_tala_db`; its focused tests and published-commit CI also passed. This local demonstration does not verify the current Cloud Run endpoint or external-host credentials.
