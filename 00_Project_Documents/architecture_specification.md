...
- Hostinger's included weekly full-VPS backups and one controlled pre-change snapshot remain supplemental infrastructure-recovery points. An independent copy must remain outside the Hostinger failure and owner-account boundary.
...
The fixed planning floor assumes one prospective production institution; one self-managed Hostinger KVM 2 VPS; the provider's included weekly backup as a supplemental copy; one client-approved institution-owned `.com` domain registered and DNS-managed through Hostinger when no suitable existing domain is available; one Hostinger Business Email Starter automated-sender mailbox; and database-backed queue/cache. Independent encrypted off-host backup remains a production requirement, but its provider and optional offline-media layer are not selected. Cloudflare R2 with client-side restic encryption and the client-owned ORICO 9548U3 with two 4 TB CMR NAS HDDs remain separately priced candidate scenarios. Hostinger paid daily backups are not selected initially. If the institution already owns an appropriate domain, a bounded subdomain is preferred and the existing domain's renewal/operations cost remains part of the institution's wider portfolio rather than a duplicate TALA purchase. `.edu.ph` is an optional later institutional-identity upgrade or alias only after PHNET eligibility evidence and current authorization exist; it is not in the baseline and does not prove accreditation. Cloud Run and any selected independent-backup service remain usage-dependent. Taxes, payment-processor fees, provider upgrades or SMTP fallback, overages, foreign-exchange spreads, shipping, retrieval, and future replacement or expansion media remain excluded until attributable evidence exists.
...
| Cloudflare R2 Standard encrypted backup repository candidate | First 10 GB-month free; then US$0.015 per additional GB-month, with published monthly operation allowances and free direct egress | **₱0.00–₱83.09 illustration** | **₱0.00–₱997.11 illustration** | Evaluated independent-backup candidate; range shows 10–100 retained GB using the dated BSP reference, before tax/FX and actual usage |
...
Hostinger displays monthly equivalents for prepaid VPS and Business Email terms. The `.com` example is a 12-month registration/renewal price and expressly excludes VAT; the email promotion and renewal equivalents use 48-month prepaid terms. Promotional entry prices are not used for the steady-state total. In the candidate illustration, Cloudflare R2 bills measured storage and operations rather than a fixed TALA plan: at the dated exchange rate, 25 retained GB is about ₱13.85/month, 50 GB about ₱36.93/month, and 100 GB about ₱83.09/month before tax/FX. Request usage must be measured. The known subtotal excludes the final independent-backup choice and remains a planning floor; actual provider selection, checkout/invoice, and consumption control the final price.
...
| --- | ---: | ---: | --- |
| Hostinger renewal floor plus R2 candidate | ₱952.92 + measured R2, illustrated at ₱0.00–₱83.09 | ₱11,435.00 + measured R2, illustrated at ₱0.00–₱997.11 | Selected steady-state KVM 2, new `.com`, and one mailbox, with R2 shown only as one independent-backup candidate; 10–100 retained GB illustrates storage before tax/FX |
...
An encrypted independent off-host repository is required before prospective production acceptance, but Cloudflare R2 is only the currently evaluated candidate. Hostinger's weekly or optional daily provider backups remain supplemental because they share the primary provider/account boundary. A larger VPS, separated workers, managed/high-availability database, or optional offline layer is budgeted only when concurrent workload, maintenance, or tested recovery evidence justifies it.
...
| Hostinger VPS daily backups | Official help depicts US$6/month and retains two recent daily plus two weekly points; hPanel checkout controls | Optional and not selected initially; quote only for same-provider restore convenience in addition to, never instead of, the selected independent off-host backup |
...
No gross annual price difference or savings amount is published because TALA's denominator is incomplete. The final independent-backup selection and other usage-dependent provider charges are unpriced, and production TCO also requires institutional operations/support, maintenance, training, migration if later authorized, security/recovery work, tax, and risk evidence. Subtracting the ₱11,435 planning floor from a commercial offer would produce arithmetic, not a defensible savings claim.
...
| --- | --- |
| Single Hostinger VPS and co-located MySQL form one failure domain | Require an independently controlled encrypted off-host repository, supplemental Hostinger backup/snapshot, measured restore evidence, and explicit scale/separation triggers. Provider procurement, named-person assignment, automation, current generations, alert delivery, optional media, and achieved restore evidence remain unproved |
...
- Keep the Hostinger account, domain, mailbox, MFA, renewal ownership/calendar, and recovery access institution-owned. Use delegated developer access rather than shared owner credentials, keep secrets only in the deployment environment, and retain an independently controlled encrypted backup outside the Hostinger failure and owner-account boundary.
- Treat the six-hour RPO and eight-hour RTO as prospective project planning targets, not law, Servitech policy, SLA, capstone acceptance, current capability, or achieved result. The deployment runbook must define the selected provider, cadence, retention, retry, alerting, key custody, and isolated-restore evidence needed to support the approved targets.
...
- Keep Hostinger, the selected backup-provider, billing, and recovery accounts institution-owned; assign named Infrastructure Custodian and privacy/DPO responsibilities; keep scoped provider credentials and controlled recovery-key copies; and do not claim operation until provider selection, procurement, automation, monitoring, and restore evidence exist.
