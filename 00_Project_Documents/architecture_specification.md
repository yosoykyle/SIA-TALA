...
1. TALA assembles an immutable snapshot from approved terms, offerings, generated scheduling demands, eligible faculty, rooms, recurring term-scoped calendar blocks, time slots, and the constraint profile. The current snapshot uses active `calendar_events` as its blocking source; no matching row means no additional availability restriction.
2. A queued Laravel job sends the snapshot to the private solver service.
3. CP-SAT validates the contract and first searches the unchanged hard-constraint model for one complete timetable. If found, the service adds a complete solution hint and the unchanged `balanced_v1` objective, then optimizes for the remaining search budget.
4. The service returns `optimal`, `feasible`, `infeasible`, `model_invalid`, or `unknown` with typed stage diagnostics. An optimization-stage limit retains the hard-valid timetable as `feasible`; a feasibility-stage `unknown` returns no assignments and makes no infeasibility claim.
...
- Keep the accepted Profile B production baseline until a separately approved promotion; use the private 4-vCPU/8-GiB candidate as the evidence-backed MIDDLE scaling option rather than claiming a universal maximum.
