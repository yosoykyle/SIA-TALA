...
The scheduling service is an isolated private Python/OR-Tools CP-SAT adapter. Source revision `cloud-cp-sat-tala-timetable-v2-lexicographic-v1-deadline-v2` uses one handler-entry 300-second request budget and a 15-second response reserve. It is not active-deployment evidence. A read-only provider check on **August 11, 2026** separately confirmed promoted private Cloud Run revision `tala-scheduler-solver-d5dstage2-665963443cc0` in `asia-southeast1`, receiving 100% of normal service traffic with 8 vCPU, 16 GiB, eight solver workers, concurrency one, a 300-second historical solver limit, a 360-second request ceiling, minimum zero instances, maximum two instances, and seed `20260718`. The service was private and its dedicated invoker authorization was present. This dated observation may drift and does not prove that the newer source is deployed, production performance, or external-host credential custody.
...
| Scheduling service | Private `tala-scheduler-solver` in `asia-southeast1`; Python 3.12 container; 8 vCPU; 16 GiB; eight solver workers; concurrency one; 360-second Cloud Run request ceiling; minimum zero and maximum two instances; seed `20260718`. Source expects `cloud-cp-sat-tala-timetable-v2-lexicographic-v1-deadline-v2` with a 300-second request budget and 15-second reserve. | Source/configuration evidence verified August 19, 2026. The last separately observed active provider revision remains the dated August 11 revision until an authorized deployment and traffic check proves otherwise. Laravel independently exposes configured identity, last observed result identity, and active deployment evidence without conflating them. |
