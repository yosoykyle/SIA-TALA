...
| **Known fixed baseline before object storage** |  | **₱812.92** | **₱9,755.00** | Honest known host-domain-mailbox floor; not the complete production cost |
...
| --- | ---: | ---: | --- |
| Hostinger renewal baseline with R2 range | ₱812.92 + measured R2, illustrated at ₱0.00–₱83.09 | ₱9,755.00 + measured R2, illustrated at ₱0.00–₱997.11 | Selected steady-state KVM 1, new `.com`, one mailbox, and required R2; 10–100 retained GB illustrates storage only, before tax/FX |
| Hostinger promotional illustration with R2 | ₱478.75 + measured R2 | ₱5,745.00 + measured R2 | ₱409/month VPS entry illustration + ₱609 first-year `.com` example + ₱19/month-equivalent mailbox promotion; provider terms are prepaid, periods differ, VAT/tax and checkout price remain excluded, and renewal controls steady-state planning |
| Existing institution-owned domain/subdomain | ₱728.00 + existing domain portfolio cost + measured R2 | ₱8,736.00 + existing domain portfolio cost + measured R2 | Avoids a duplicate domain purchase; arithmetic includes KVM 1 and one mailbox while the institution retains its existing domain renewal and DNS obligations |
...
| --- | ---: | ---: | ---: | --- |
| **TALA known planning floor before object storage** | Per-institution host/domain/one-mailbox floor; not normalized by learner count | **₱812.92 + unquoted and usage-dependent costs** | **₱9,755.00 + unquoted and usage-dependent costs** | Not complete production TCO; required object storage, operations/support, usage, transactions, tax, and risk remain unresolved |
...
- Select an institution-owned Hostinger account as the lean initial production control surface: KVM 1 for the self-managed Laravel/MySQL/initial-queue topology, Hostinger DNS for a client-approved institution-owned `.com` domain when no suitable existing domain exists, and Business Email Starter for one dedicated automated-sender mailbox. Prefer a bounded subdomain of an appropriate existing institution-owned domain over a duplicate purchase. This centralization is a bounded starting choice, not a claim that Hostinger is universally superior.
