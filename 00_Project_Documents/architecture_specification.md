...
Not currently installed: `laravel/horizon`. Removed in TAL-93A as unused: `pxlrbt/filament-activity-log` (superseded by the hand-built read-only `ActivityResource`) and `maatwebsite/excel` (imports and exports use native `fgetcsv`/`fputcsv`, not an Excel library). Redis support is available through Laravel's standard configuration if the runtime provides the Redis PHP extension or a compatible client, but Redis/Horizon are not the active queue baseline.
...
* **Detailed Comparison:**
  ...
  | :--- | :--- | :--- | :--- |
  | **Constraint Guarantee** | **Model-enforced and independently validated.** CP-SAT can enforce correctly encoded hard constraints, while TALA must independently reject incomplete, invalid, partial, or contract-breaking results before publication. | **Probabilistic.** ML models output predictions. They cannot guarantee that a teacher won't be assigned to two classes simultaneously or that room capacities won't overlap. | **Heuristic.** GAs improve schedules over generations, but they can get stuck in local optima and output a schedule that still violates minor hard constraints. |
  ...
  | **Adaptability to Rule Changes** | **Explicit model update.** Approved rule changes are encoded in the constraint model and independently tested without training data; effort depends on the rule and its interactions. | **Requires Retraining.** Learned behavior generally requires suitable data plus model validation and possibly retraining when scheduling rules change. | **Requires Fitness Rewriting.** Rule changes require corresponding fitness and feasibility updates plus renewed tuning and testing. |
  | **Proof of Optimality** | **Status-dependent.** CP-SAT can prove optimality when it returns `OPTIMAL` and can prove model infeasibility when it returns `INFEASIBLE`; a time-limited `FEASIBLE` or `UNKNOWN` result does not provide either proof. | **No.** ML outputs a single generated recommendation with no measure of mathematical optimality or feasibility. | **No.** GAs stop after a set number of generations, with no proof of whether a better schedule exists. |
