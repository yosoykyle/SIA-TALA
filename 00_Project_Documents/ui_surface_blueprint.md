...
| 06 CP-SAT Scheduling | Demand readiness, solver run, candidate review, publication, revision | Benchmark-selected accessible review surfaces with a canonical table, plus focused run, publish, and revision actions | Reuse existing run/candidate inventory after contract audit; select the minimum proportionate presentation only after TAL-94 independently benchmarks operator needs |
