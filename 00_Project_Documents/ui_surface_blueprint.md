...
| 06 Constraint Programming–Satisfiability (CP-SAT) Scheduling | Demand readiness, solver run, candidate review, publication, revision | Accessible canonical tables plus focused run, publish, and revision actions; later TAL-96D may group status, coverage, hard-constraint validation, objective/bound/gap, and runtime under **Solution Quality** | Reuse existing run/candidate inventory; corrected TAL-96B4 evidence supplies empirical terminology and does not authorize an accuracy percentage, capacity claim, chart, or scoring-control UI |
...
| Candidate review | Candidate rows | Relation-manager Table with filters, warnings, validation status, and a plain-language Solution Quality summary sourced from typed solver evidence when TAL-96D approves the UI change |
