...
| BM-10 | Laravel Excel / Maatwebsite Excel documentation. | Controlled CSV/XLSX imports and exports, chunking, and queued processing for templates, rosters, and reports. |
...
| BM-21 | DepEd Order No. 8, s. 2015. | Authoritative SHS classroom-assessment component weights, transmutation, reporting, and Grades 11-12 progression/remediation baseline. Institution summaries that conflict with the order are not implemented literally. |
...
| Person and student master record | BM-01, BM-02, BM-04 | Permanent student folder and school record are central to every office workflow. | Maintain one canonical person/student profile with identifiers, demographics, education level, program/strand, status, and linked evidence. | UAT Core |
| Academic calendar, school year, and term setup | BM-01, BM-04, SIA-01 | SHS and College can have separate term/calendars and gates. | Model academic years/terms/calendar phases explicitly; do not hardcode one global school year. | UAT Core |
...
| School-year/term enrolled-student inventory | BM-01, BM-02, BM-04, BM-10, SIA-01 | School needs to know who is enrolled per term/batch/section; external encoding happens outside TALA. | Provide authorized roster/inventory by academic year/term, level, program/strand, year/grade, section, delivery group, and status. Generic CSV/XLSX export is allowed; DepEd/CHED/LIS portal submission/tracking is not a TALA module. | UAT Core/Supporting |
...
| Academic progression, irregular students, and subject suggestion | BM-01, BM-04, SIA-01 plus regulatory benchmark where applicable | SIA has SHS/College progression and irregular/manual evaluation flows. | Use grade history, prerequisites, curriculum gaps, failed/INC/back subjects, and versioned policy. Regulator-conflicting rules must be benchmarked before automation. | Supporting/UAT Core if demo includes continuing student |
...
| Student status lifecycle | BM-01, BM-02, SIA-01 | No-show, LOA, returnee, archive/reactivate, transfer-out affect records. | Status transitions must be typed, reversible/terminal where appropriate, reasoned, audited, and separate from staff account archive. | Supporting/Phase 2 |
| Graduation evaluation and completion boundary | BM-01, SIA-01 | Graduation review checks academic history, clearances, deficiencies, and external CHED/SO process. | TALA should support graduation eligibility/evaluation snapshot and completion status. Diploma/CHED/SO external submission is outside first UAT unless explicitly promoted. | UAT Core boundary; Phase 2 for full credential release |
| Attendance, behavior, discipline, and guidance | BM-01, BM-05, SIA-01 | Business workflow names these as gates, but current data source/process is not yet complete. | Do not silently infer these gates. Add typed evidence/cases/privacy/appeal/resolution before enforcing enrollment blocks. | Phase 2 / Benchmark Gate |
...
| Imports and legacy migration | BM-10, BM-01, SIA evidence files | Legacy sheets/forms support startup data and audit. | Use controlled templates, private files, validation preview, zero-error commit where required, import batch audit, and normalized accepted rows. | Supporting/UAT Core for data needed by demo |
| Exports and reports | BM-01, BM-10, SIA-01 | Staff may need roster, schedule, SOA, grade, and generic report outputs. | Export only authorized fields from authoritative queries. CSV/XLSX/PDF outputs are evidence/artifacts, not operational source of truth. | Supporting |
...
17. Import/export baseline requires versioned templates, private sources, validation preview, explicit audited commit, field allowlists, and generic artifacts rather than portal-specific automation.
18. Attendance, behavior, discipline, and guidance remain a benchmark gate until typed evidence, ownership, privacy, appeal, and resolution policies exist; missing data must never silently block enrollment or completion.
