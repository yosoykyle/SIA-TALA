...
Feature Group 5 passed its deep submission-lock audit on 2026-06-21. Existing SDD-06A-D posting behavior remains valid, but "closed" applies only to those implemented slices, not the complete finance goal state. SDD-06E owns effective-dated fee structures and assessment snapshots, SOA/payment-evidence artifacts, materialized-balance rebuild proof, duty segregation, private reminders, and daily three-way reconciliation; SDD-06F owns component-level disposition/refund execution. BIR official receipt/invoice/e-receipt/CAS generation is removed from TALA scope. Assessment must stop writing `enrolled_at`, which remains an official-handover field under SDD-07A.

Feature Group 7 was rebaselined on 2026-06-21. COR remains the UAT-core generated academic artifact after canonical enrollment. The document-request portal/catalog/fee/fulfillment/courier domain is permanently removed under SDD-00D. SDD-07A/SDD-08A own COR issuance/PDF/QR/public verification and dedicated COR permission cleanup; SDD-06E owns SOA/payment evidence and external receipt/reference recording; SDD-07E owns diploma/certificate release after graduation evaluation. Other generated artifacts remain owned by their authoritative source workflows.
...
### Scope Pruning and Role-Feature Rebaseline Overlay (2026-06-22)

`SDD-00E Scope Pruning and Role-Feature Rebaseline` pauses `SDD-03R/TAL-31` until the selected client removals and external boundaries are reflected in active docs, UAT rows, local checklist, and Linear. The removed/clarified items are: cross-enrollee intake (`F18`), DepEd/CHED/LIS portal submission/status tracking (`F39`, `INT09`), BIR/CAS/e-receipting and official-receipt generation (`FIN15`, `INT10`), courier/shipping automation (`INT11`), and LMS/Zoom/Google Meet link handling (`INT12`). After this cleanup, the next planning pass is a role/module feature review that ranks only the College SIS foundation and approved integrations by dependency and UAT value.
...
### SDD-00E: Role/Module Feature Rebaseline

**Goal:** Recompute the active backlog after the client-selected removals so in-progress work, next work, and future backlog are selected from the College SIS foundation rather than stale feature lists.

**Backlog effect:** `SDD-00E` redefines the roadmap classification, not the source-code history. Completed compatible code remains evidence. In-progress work can be blocked or reprioritized. Backlog rows that depend on cross-enrollee, document-request/courier, DepEd/CHED/LIS portal tracking, BIR/CAS/e-receipting, or LMS/meeting-link automation are removed or rewritten as external-boundary evidence/export work.

| Role/module lane | Keep as active SIS foundation | Remove or externalize | Priority after SDD-00E | Next sprint source |
| --- | --- | --- | --- | --- |
| System Super Admin / Identity | Staff/user lifecycle, one-role access, protected routes, audit visibility, safe logout/session behavior | No feature-specific expansion unless a security/UAT defect appears | P0 regression guard | SDD-04 only if login/RBAC/UAT seed fails |
| Registrar admissions and records | Published College offerings, requirement policies, applicant checklist, OCR/manual review, retention undertakings, Registrar-assisted legacy/readmission setup, student record jacket | Cross-enrollee intake, SHS offerings, document-request catalog/fulfillment, DepEd/LIS/CHED portal status | P0 | SDD-07A |
| Registrar enrollment and COR | Canonical enrolled state, payment/capacity/placement handover, school-year/term enrolled roster/export, COR issuance/PDF/QR verification, dedicated COR permissions | LIS encoded/error status, mark-encoded action, portal credentials, regulator-specific templates | P0 | SDD-07A plus LIS cleanup |
| Registrar scheduling/classes | College terms, curricula, sections, delivery groups, faculty eligibility/availability, CP-SAT solve/review/commit/publish, approved post-publication Apply | LMS/Zoom/Google Meet link storage/export; direct edit after publish | P0 integration | SDD-03R/TAL-31 after SDD-00E |
| Accounting finance | Assessment, downpayment clearance, PayMongo/manual payment parity, immutable ledger, promissory promise tracking, exam-access accommodation, SOA/payment acknowledgement, external receipt/reference evidence | BIR official receipt, invoice, e-receipt, CAS, receipt-number issuance, document-request/shipping fees | P0 for handover blockers; P1 for reconciliation/refund depth | SDD-06E/06F |
| Faculty grades | Assigned class lists, College grade encoding, Registrar verification/finalization, correction workflow, grade history | Finance-derived class-list exposure, attendance/debt/LMS restriction effects | P0 | SDD-08A |
| Academic Head | Curriculum readiness oversight, schedule publication, workload exceptions, grade-change approval, read-only cross-module oversight | Direct Accounting collection/payment mutation; unsupported broad admin shortcuts | P0/P1 by dependency | SDD-01/03/08A as needed |
| Student Hub / Applicant UI | Applicant intake UI, protected read-only Student Hub dashboard/schedule/grades/financials/COR/help, clear loading/error/offline-denial states | Document-request UI, student mutation flows not backed by approved services, advanced PWA cache polish before backend stability | P0-lite after backend services; P2 polish | TAL-13/SDD-08B after P0 backend |
| Imports/exports/reports | Curriculum import, generic enrolled roster export, source-data exports needed for manual external work | DepEd/CHED/LIS submissions, portal queues/statuses, official school forms as TALA-owned filings | P0 only for enrolled roster; P1 reports | SDD-07A then SDD-09 |
...
| **SDD-06E - Finance operations and reconciliation delta** | FS 6.1-6.3; TS 3.12/3.14 | Fee structures, Collector/Recorder/Verifier, manual/online payment evidence, daily payment reconciliation, private reminders, clearance/SOA | Immutable payment/ledger services exist; fee template is transitional and no assessment snapshot, SOA/payment-evidence issuance, maker-checker daily close, or projection rebuild exists | Preserve existing posting services. Add versioned effective-dated fee structures/component lines, assessment snapshots, policy-driven discount, duty permissions, SOA/payment-evidence issuance, balance rebuild check, expected/actual close, variance reason, verifier approval, and private reminders. Remove assessment-owned `enrolled_at`. |
