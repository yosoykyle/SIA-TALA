...
Finish `SDD-00C College-Only Scope Correction` verification, Linear mirror, and commit first. After SDD-00C is closed, resume SDD-07A remaining work: tentative placement expiry, canonical enrollment-state cleanup, enrolled-roster export, extension/reminder UI, and notification delivery.
...
- **Active target:** SDD-00C is in verification/closure because the institution removed SHS from active scope. SDD-07A implementation remains paused until the College-only cleanup is mirrored to Linear and committed. Completed SDD-07A backend/admin evidence remains preserved where compatible with College-only flow.
