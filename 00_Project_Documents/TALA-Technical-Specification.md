...
**External-System Technical Boundary:** TALA must not implement DepEd, CHED, LIS, BIR/CAS/e-receipting, courier, LMS, Zoom, Google Meet, or other external-portal workflow automation, credential storage, queue/status tracking, or API mutation. The technical baseline supports internal evidence, authorized school-year/term enrolled-student roster visibility, and generic exports for staff manual external work.

**Receipt and Tax Technical Boundary:** TALA is a student subsidiary ledger and internal evidence system only. It may store externally issued receipt/reference evidence and issue SOA/payment-acknowledgement artifacts, but it must not generate BIR official receipts, invoices, or e-receipts in the active baseline.
...
- A confirmed collection may record externally issued receipt/reference evidence with unique reference metadata, payment links, channel, payer, amount, recorded actor/time, source evidence, checksum, and correction state. TALA does not generate BIR official receipt, invoice, or e-receipt numbers, PDFs, or tax documents.
