...
**Current TAL-12 Implementation Status**: The faculty availability tables and permissions exist as scheduling foundation. Full `FacultyAvailabilityService`, faculty submission UI, Registrar review/lock UI, and change-request screens are not counted as completed Pre-UAT evidence until implemented and tested. Current TAL-12 admin QA may validate scheduling resources, term readiness boundaries, schedule-generation resources, and access control only.

**Implementation Components**:
- `TermSchedulingReadinessService`: checks whether a selected term has `term_name`, `term_start_date`, `term_end_date`, and `scheduling_starts_at`; returns `is_ready`, `missing_fields`, and blocking messages for the UI.
...
- `ScheduleGenerationService`: reads locked availability, sections, subjects, rooms, and modality constraints to create `schedule_generation_runs` and `schedule_draft_rows`.
- `ScheduleConflictValidator`: detects teacher double-booking, room double-booking, teacher outside locked availability, invalid time ranges, and modality violations.
...
| --- | --- | --- | --- |
| `TermSchedulingReadinessService` | `check(Term $term): TermReadinessResult` | Persisted `Term` model | `{is_ready: bool, missing_fields: string[], blocking_messages: string[]}` |
...
| `ScheduleGenerationService` | `generate(Term $term, User $registrar): ScheduleGenerationRun` | Ready term and authorized Registrar | Creates one draft run with rows in `generated` or `conflict` status |
...
3. Registrar creates one availability period per term with `opens_at` and `closes_at`; validation enforces `opens_at < closes_at <= scheduling_starts_at`.
4. Faculty can edit only while the period is open and their submission is `draft` or `revision_requested`.
5. Submitted/locked availability cannot be edited directly. Late or exceptional changes require `faculty_availability_change_requests` with reason and Registrar approval.
6. Schedule generation is manually started by the Registrar. Faculty submission never auto-generates or auto-commits an official schedule.
7. Automatic generation includes only faculty with valid `submitted` or `locked` availability for the selected term unless Registrar records an override reason.
8. Every generation is saved as a draft run. Multiple runs can exist as `generated`, `under_review`, `abandoned`, `superseded`, or `committed`.
9. Hard conflicts block commit. Registrar resolves conflicts by editing teacher, time, room, or section schedule; Faculty may be consulted but does not resolve conflicts in the system.
10. Official schedules come only from committed `section_meetings`. Post-commit changes require `schedule_changes` with old/new payloads, reason, approval, and audit trail.
...
**Service**: `DocumentRequestFulfillmentService`
...
-   `markShipped(DocumentRequest $request, array $data, User $registrar)`: validates `processing` state and delivery consent, requires `courier_name`, `shipping_fee`, `courier_receipt_path`, and `tracking_number` or `N/A`, then transitions to `pending_shipping_payment`.
...
| `courier_receipt_path` | required private file path from upload validation | Upload accepts `jpg`, `jpeg`, `png`, or `pdf`, max 5 MB, stored on a private disk |
...
**Current TAL-12 Implementation Status**: `DocumentRequest::documentTypeOptions()` and the Filament document request form enforce the fixed approved selectable catalog. A dedicated document-catalog CRUD/settings UI for Registrar metadata and Accounting fee-policy maintenance is not yet part of the implemented Pre-UAT scope; add it as a separate implementation item before testing catalog administration.
...
FAQ table migration is already prepared (`faq_entries`) and tracked in the Foundation Migration Control Log. Execution is intentionally staged with the FAQ module rollout.

#### 3.16.2 System Super Admin Interface (Filament)

-   **Resource**: `FaqResource` in the Filament staff panel
-   **Features**: CRUD for FAQ entries, drag-and-drop reordering (`sort_order`), toggle `is_published`, filter by `category`
...
#### 3.16.3 Public & Student Views (Livewire)

-   **Route**: `GET /faq` (public, no auth required)
-   **Component**: `App\Livewire\Public\FaqPage`
-   **Features**: Searchable FAQ list filtered to `is_published = true`, grouped by `category`, ordered by `sort_order`
-   **Student Hub**: The same FAQ data is also linked from the Student Hub’s HOME tab as a “Help & FAQ” quick link
...
#### 3.16.5 Migration Logging Rule

-   FAQ storage remains a deferred table item until the module implementation sprint starts.
-   Before creating/running the FAQ migration, add/update its status row in `TALA-Foundation-Migration-Control-Log.md`.
-   This keeps migration execution history and spec policy history synchronized.
...
The generic `SystemSettingResource` is hidden from Filament navigation and direct generic resource access is denied during TAL-12 Pre-UAT. This prevents testers from seeing disabled/read-only infrastructure keys or raw JSON fields that are not part of an approved role workflow.
...
**Current TAL-12 Implementation Status**: `ImportBatchResource` is intentionally limited to an audit surface named **Import Batch Audit**. It has no generic create/edit resource routes or actions. It may view batches and expose approved commit/cancel state transitions for existing pending batches, but the dedicated upload, parse, preview, error-report, and commit pages listed below remain a separate implementation item.
