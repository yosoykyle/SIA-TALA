...
`PaymentConfirmationService` delegates finance-cleared account handover to `StudentEnrollmentService` after the Accounting minimum-downpayment/full-payment rule moves an enrollment to `pre_enrolled`. The handover sets `users.status = 'active'`, switches `users.username` to the generated student ID, removes the `applicant` role, assigns the `student` role, records audit evidence, and exposes `corReadiness()` for COR/class-list gates. Current manual Accounting confirmation follows this path. PayMongo webhook processing remains a payment/ledger posting path until the SDD-06 payment slice wires equivalent finance-clearance evaluation and handover.
