...
| Table | Key columns and lifecycle | Required constraints and indexes | History/deletion |
| --- | --- | --- | --- |
| `fee_rules` | Unique fee code, name, ledger/display category, Program/Term scope, calculation type, fixed amount or per-unit PHP rate as `DECIMAL(12,2)`, effective dates, active state, authority. | Unique active code/scope/effectivity; indexes applicability, category, and state. Ordinary charges resolve by scope rank, then effective date and ID; downpayment activation requires exact Program and Term scope. | Supersede/end-date; assessment lines retain rule/version. A separate fee-item identity table is unnecessary for the bounded MVP matrix. |
