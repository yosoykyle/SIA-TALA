...
| `Optimal` or `Feasible` candidate | Validate solver result; request a local adjustment or explicit minimal repair | TALA, then Registrar reviews | Server-side validation plus Registrar review authority | Every hard rule independently passes; a requested adjustment is fixed exactly | Immutable candidate successor and quality/diagnostic/change evidence | Local adjustment changes only the requested meeting; repair may change others only after full preview and whole-candidate acceptance; candidate is never official | Registrar/Academic Head see candidate and exact impact; Faculty/Students do not |
...
The accepted architecture targets the private `tala-scheduler-solver` Cloud Run service in `asia-southeast1`, where the accepted Issue #22 activation dated **August 19, 2026** qualified revision `tala-scheduler-solver-i22-3f461a46`: 8 vCPU, 16 GiB, eight solver workers, concurrency one, a 300-second solver limit, a 360-second HTTP timeout, minimum zero instances, maximum two instances, and deterministic seed `20260718`. While this dated activation evidence is preserved, current live external Cloud Run service state is unverified in this session.

**Implementation and Test-Evidence Boundary:** CP-SAT timetable generation is TALA's principal optimization capability. Local solver execution and orchestration is implemented via Laravel commands and queue jobs, supported by automated unit and feature test suites with mock fixtures (`Unit/TAL65CloudRunSchedulingSolverClientTest.php`, `Feature/TAL96B2RealLoopbackSchedulingDemoReadinessTest.php`). Live local solver execution requires local environment execution, while external Cloud Run integration remains unverified for live production state.
...
The solver produces the first complete candidate. Registrar may use **Adjust candidate meeting** with constrained day, time, Faculty, and room choices. TALA first fixes every other meeting and tests the requested replacement against the entire candidate. If every hard rule passes, it creates an immutable candidate successor that changes only the requested meeting. If not, it saves nothing and shows the attributable conflicts.

From that failed local attempt, Registrar may explicitly choose **Find valid repair**. Repair fixes the requested meeting as a hard requirement, solves the whole Term, and first minimizes the number of changed non-requested meeting assignments before applying the ordinary fixed quality hierarchy. A meeting assignment is its day, time, Faculty, room, and delivery location tuple. Solver hints may support search but never constitute this guarantee.

Before acceptance, TALA previews every changed meeting, the source candidate, the repaired quality measures, and any quality difference. Registrar accepts or rejects the whole immutable successor; no other meeting moves silently. Every local adjustment or repair revalidates the entire candidate, no hard constraint can be waived, and a successor that lowers ordinary soft quality requires a publication reason. “Manual override,” generic constraint-bypass records, editable repair weights, and generic solver profiles are removed.
...
- Local candidate adjustment that either changes only the requested meeting or saves nothing with attributable conflicts.
- Explicit whole-Term minimal repair that fixes the requested meeting, minimizes changed non-requested meetings before ordinary quality, previews every affected meeting, and applies only after whole-successor acceptance.
...
| Solver retry/correction | Registrar | Same-snapshot retry only for `Unknown`; `Optimal`/`Feasible` need source change or recorded rejection; `Infeasible` needs conflicting fact change; `ModelInvalid` defect correction; `TechnicalFailure` recovery | Local correction first fixes all other meetings and either creates a one-meeting successor or changes nothing. Explicit **Find valid repair** fixes the request, minimizes changed other meetings before ordinary quality, previews all changes, and creates an immutable successor only after acceptance. Every path revalidates hard rules; stale/invalid work changes nothing |
