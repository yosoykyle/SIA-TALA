...
- The scoped [PUP 2026–2027 overlap schedule](https://www.pup.edu.ph/announcements/?go=Cjoh4ZVj%2FLE%3D&v=Schedule-of-AY-2026-2027-First-Semester-Online-Enrollment-and-Encoding-of-Grades-20260727134235133), which demonstrates prior-term grade encoding during next-term enrollment/adjustment but supplies no Servitech deadline or grade rule.
- The [PUP request to reopen a grading sheet](https://www.pup.edu.ph/downloads/forms/PUPSISRFROG.pdf), which demonstrates a scoped authority-and-reason pattern for late academic-record work. Its `P` usage and PUP-specific consequences are not adopted by TALA.
- [PUP Graduate School grading guidance](https://www.pup.edu.ph/gs/Grading), which requires completion of an `INC` within one year and then uses a retake-or-no-credit consequence. It is qualified comparison evidence for TALA's bounded default, not Servitech policy or authority for PUP-specific enrollment restrictions.
- [PeopleSoft enrollment requirement groups](https://docs.oracle.com/en/applications/peoplesoft/campus-solutions/9.2.038/student-records/defining-enrollment-requirement-groups.html), which shows that institutions may include or exclude in-progress coursework from requisite evaluation. TALA chooses the conservative completed-and-released rule rather than importing a configurable policy engine.
...
- **Sources consulted:** the Servitech final-grade workbook, Batas Pambansa Blg. 232, CHED eCAV requirements, the cited scoped PUP/UP comparisons, PeopleSoft grade-roster self-service, the legacy TALA PRDs, and bounded code/schema implementation evidence.
...
- **Rejected or bounded:** another institution's rule is not Servitech evidence. TALA adopts only the explicitly bounded one-year nonautomatic INC completion default: after an unextended deadline the original `INC` remains no-credit and the course must be retaken. No external automatic grade conversion, academic sanction, or load rule is copied. Period-grade formulas, attendance, generic progress engines, editable transcript templates, clearance routing, delivery, and diploma production remain outside TALA.
...
- The same current official membership supports one contextual operational Class Roster print and one fixed CSV for assigned Faculty and authorized Registrar use; neither artifact is a gradebook, official-output issuance, or independent roster record.
...
Registrar release rechecks roster membership, submitter authority, completeness, current enrollment effects, and stale versions. It publishes the official results atomically, recalculates academic/curriculum/prerequisite projections, creates one idempotent impact review for every affected active Clinic 4 Registration Case, and queues the grade-release notice only after commit. This applies to the first official release as well as a later INC resolution or correction; no released result silently changes enrollment.
...
| `CompletionOverdue` | The current deadline has elapsed and no successor result exists | Registrar may append an authorized deadline extension; otherwise the original result remains `INC` with no credit and the Student must retake the course |
...
- Deadline passage is a derived state and audit-timeline fact, not a grade event. TALA never converts the result to `5.00` or another grade automatically. Without an authorized extension, ordinary completion closes, the original `INC` remains no-credit, and the Student must retake the course.
- While `CompletionOpen`, Faculty records the authorized completion result and Registrar releases the immutable superseding Official Grade Event. The original `INC`, deadlines, amendments, overdue interval, and final result remain historically visible.
...
- There is no arbitrary completion-attempt count while `CompletionOpen`. Deadline amendments remain append-only and require exact authority; an overdue result cannot be completed through the ordinary path unless an authorized extension first returns it to `CompletionOpen`.
...
- Student Academics shows the course, completion note, current deadline, `CompletionOpen` or **Completion overdue**, responsible Registrar office, and next safe action. Faculty sees the assigned-course completion action only while open; Registrar sees the complete authority/history and extension or retake path; Academic Head remains read-only.
...
- Any affected active Clinic 4 Registration Case enters the same deterministic Registrar review used for an initial release or INC resolution. TALA never silently adds, removes, replaces, or assumes that an officially registered course should remain.
...
Draft and submitted-but-unreleased grades never affect registration. An unreleased, failed, incomplete, withdrawn, or dropped prerequisite leaves its dependent course ineligible while unrelated eligible courses remain available. TALA creates no provisional prerequisite permission or substitute grade.

Every transition from `Pending` to a released or otherwise authoritative projection—including initial release, INC resolution, correction, or exact approved-credit mapping—recalculates the curriculum/requisite facts and routes affected active Registration Cases by exact version. `Satisfied`, or `ApprovedCredit` that exactly satisfies the named prerequisite, makes the dependent course eligible but never adds it automatically; Clinic 4 uses the ordinary guarded Adjustment path. `Unsatisfied`, `Incomplete`, or `WithdrawnOrDropped` keeps the dependent course excluded. If a later correction invalidates a prerequisite after Official Enrollment, that enrollment remains intact pending Registrar review and any separately authorized Clinic 4 Adjustment or Course Drop. A failed result alone does not automatically impose probation, dismissal, a reduced load, or an enrollment mutation.
...
An academic-result impact review is a routed projection, not another academic sanction or approval workflow. It identifies the released source/version, affected Registration Case and dependent course, before/after requisite fact, whether the case is pre- or post-finalization, responsible Registrar office, available Clinic 4 action, and as-of time. Duplicate delivery returns the same review. The review closes when the proposal is validly revised, a newly eligible course is handled through ordinary Adjustment, or an authorized Clinic 4 outcome is recorded; absence of late-adjustment authority preserves the current Official Enrollment and names the next permissible path.
...
| `INC: CompletionOverdue` | Deadline passes with no successor result | System-derived; Registrar owns any authorized extension and the retake guidance | Own-record or academic-record authority; Faculty completion action is unavailable while overdue | Current unresolved INC; latest deadline elapsed | **Completion overdue** guidance and audit timeline; grade remains `INC` with no credit | Authorized future extension may reopen the window; otherwise a retake creates its own course attempt and result without overwriting the original `INC` | Dependent courses and completion remain pending; unrelated enrollment is not globally blocked |
...
| INC deadline and completion | Original `INC`, official Term end, current deadline/amendments, current time, completion result | Registrar owns deadline corrections/releases; Faculty supplies completion only while open | One-year deadline calculated; latest amendment current; unresolved source/version current | Stale action records nothing; overdue state permits only an authorized extension or retake path; grade never changes automatically | Display current state, amend deadline, release an open completion, or direct the overdue Student to retake | Correct the exact stale source, append an authorized deadline extension, complete while open, or preserve the original no-credit `INC` and create a separate retake attempt |
...
The primary actions are **Save draft** and **Submit complete roster**. **Print class roster** and **Export roster CSV** are contextual secondary actions available to every assigned Faculty member who may view that selected roster and to an authorized Registrar; view-only co-Faculty still cannot edit or submit results. Returned-row correction, history, and evidence remain secondary actions. Native Filament tables and forms are used; spreadsheet import and a custom gradebook are excluded from MVP.

Both roster actions re-read the current Clinic 4 official membership and apply record- and row-scoped authorization at generation time. The A4 landscape browser print is titled **CURRENT CLASS ROSTER — Operational reference** and uses practical 12 mm margins, repeating institution/roster/table headings, deliberate row-safe page breaks, and automatic page numbers when multiple pages are produced. It shows institution identity, exact Term, Class Offering reference, Course code/title, program/cohort context, assigned Faculty, current official learner count, generation time in Asia/Manila, and rows ordered by legal name then Student number. The fixed CSV is named `tala-class-roster-{class-reference}-YYYYMMDD-HHmmss-PHT.csv` and has these ordered labels: `Term`, `Class Reference`, `Course Code`, `Course Title`, `Program or Cohort`, `Student Number`, `Legal Name`, `Official Enrollment State`, `As of (Asia/Manila)`.

The print and CSV contain no grade, result, note, address, phone, email, Applicant, or finance field. They are current as-of operational extracts, not immutable official outputs or proof of enrollment. Output access records actor, selected Class Offering, fixed role-derived purpose, source/as-of time, row count, and outcome. CSV cells are formula-safe. With no officially enrolled learners, the roster shows **No officially enrolled students in this class** and offers no print/export action. A stale, inaccessible, changed, or failed source creates no partial file or printable artifact; the roster remains available with **Refresh roster** or the shared safe return/support path. Retrying re-reads the current source and sends no email.
...
| Registrar releases roster | Affected Students | Official results available; secure link; when applicable, **Registrar review is required for your enrollment** and an authenticated Enrollment link | Released roster version plus Student identity | Results remain official in Student Academics; review remains visible even if mail fails | No grade value, attachment, or duplicate review email |
...
| `INC` resolved | Student | Outcome available in Student Academics; secure link; applicable enrollment-review status | Superseding result event plus Student identity | Result/recalculation/review remain committed | No grade value or attachment; deadline passage alone sends no email |
| Authorized correction released | Affected Student | Academic record updated; secure history link; applicable enrollment-review status | Correction release event plus Student identity | Correction and review remain committed | No old/new grade value, attachment, or separate review email |
...
| AcademicResultImpactReview | Route a released result's effect to an exact active Registration Case | PRD 05 derives; PRD 04 Registrar resolves enrollment action | Derived projection/calculation | Registrar and affected Student | One idempotent cross-clinic projection, not a sanction, approval aggregate, or duplicate Registration Case |
...
| Curriculum position, enrollment effect, and active-case impact | Curriculum/result/lifecycle sources; ordinary or nonstandard placement basis; any actual decision authority/explanation/effective term; current Clinic 4 effect; affected active Registration Case/course and pre/post-finalization disposition | Factual projections are generated; consequential external decisions are Registrar-recorded and append-only; no result event mutates Clinic 4 directly |
...
- INC release with the calculated one-year deadline, leap-date handling, amendment, `CompletionOpen`, `CompletionOverdue`, authorized extension or separate retake path, stale/concurrent action, academic-average withholding, prerequisite/advising effects, and no automatic grade conversion;
- correction without a hard cutoff, append-only history, active-registration review, and issued-TOR supersession;
- initial release, INC resolution, and correction producing the same idempotent active-case impact review, including pending-prerequisite exclusion, later eligibility, and post-enrollment adverse correction outcomes;
...
- one unreleased prerequisite whose dependent course is excluded, one later satisfying release handled through ordinary Adjustment, one adverse correction after Official Enrollment while Adjustment is open, and one post-closure case with and without late-adjustment authority;
...
7. Pass the inclusive deadline for another `INC`; confirm **Completion overdue**, no grade mutation or overdue email, dependent-course/completion blocking only, no ordinary Faculty completion action, and clear no-credit/retake guidance. Append an authorized future extension and confirm it returns to `CompletionOpen` without erasing the overdue interval.
8. Record an authorized correction; confirm append-only history, recalculated projections, and the same idempotent Registrar review used by initial release and INC resolution on any affected active Clinic 4 Registration Case.
...
16. For an active next-Term Registration Case, keep the dependent course excluded while its prerequisite is unreleased, then release a satisfying result. Prove that eligibility changes without an automatic add and that ordinary Adjustment rechecks confirmation, capacity, schedule, load, and Finance. Separately apply an adverse grade correction after Official Enrollment and prove that one review opens; open Adjustment uses the guarded change path, closed Adjustment requires exact late authority, and absence of authority preserves enrollment/COR and names the next path. Duplicate events, failed email, stale Finance, capacity conflict, and retry never create a second review or partial mutation.
17. From one selected official roster, verify that the designated Faculty, view-only assigned co-Faculty, and authorized Registrar can print and export only that current class membership; an unrelated Faculty member cannot access either action. Confirm the A4 landscape print, 12 mm margins, repeated identity/table headings, automatic numbering across multiple pages, fixed columns/order, formula-safe CSV cells, the operational-reference/as-of labels, output-access evidence, synchronized membership after adjustment or Course Drop, no grade/contact/finance data, and no partial artifact for empty, stale, inaccessible, changed, or generation-failure states.
...
|---|---|---|---|---|
| Current Class Roster print/CSV | Assigned Faculty with roster-view access or authorized Registrar; authorization rechecked for the selected Class Offering and every row | Current Clinic 4 official membership; exact Term/Class Offering; nonempty source; fixed columns and ordering | One-class operational extract only; fixed role-derived purpose and output-access evidence; no grades, contact, finance, column chooser, alternate format, bulk selection, or official-output event | Empty roster offers no action. Stale, changed, inaccessible, or failed source creates no partial artifact; refresh/retry re-reads current membership and never sends email |
...
| Released-result active-case impact | TALA derives from an initial release, INC resolution, or correction; Registrar owns any Clinic 4 action | Exact result/event, prerequisite/curriculum versions, active Registration Case/course, and pre/post-finalization state | Student/Registrar see source, affected course, owner, and next action; no grade value appears in mail and no new navigation/workflow aggregate is created | Duplicate/stale delivery creates no second review. Newly eligible courses require ordinary Adjustment; invalid pre-finalization facts become stale; Official Enrollment remains until an authorized guarded change |
...
| INC deadline and amendment | System derives one year from Term end; Registrar may amend with authority | Original Term end, inclusive Asia/Manila calculation, current unresolved source/version; amendment requires prior/new date, authority/date, reason, actor | **Change INC deadline** shows the current/new deadline, resulting open/overdue state, email, and append-only consequence | Deadline passage derives `CompletionOverdue` but never a grade/email. Ordinary completion closes until a valid future extension reopens it; otherwise the original `INC` remains no-credit and the Student retakes the course |
...
The INC deadline and overdue behavior are complete TALA product rules. No automatic grade conversion or grading-policy engine exists; an overdue unextended `INC` remains no-credit and requires a separate retake. TALA Standard TOR — Servitech v1 is ready for Registrar preview and issuance when its operational data and source readiness pass. Physical signing, sealing, claiming, delivery, courier, CAV, diploma, and ceremony remain external and do not leave product behavior unresolved. Personal-data handling follows the product-wide no-automatic-disposal boundary.
## 15. Assumptions
...
- The supplied Servitech evidence does not establish an INC deadline or automatic lapse rule. TALA therefore adopts the bounded one-year deadline and the official PUP Graduate School's retake-or-no-credit consequence as a qualified project default, while rejecting PUP-specific automatic conversion or enrollment sanctions.
