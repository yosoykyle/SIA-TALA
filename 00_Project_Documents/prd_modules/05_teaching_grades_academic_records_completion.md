...
- The [UP November 2020 memorandum](https://publicservice.up.edu.ph/wp-content/uploads/2020/11/OVPAA-MEMO-2020-139_15-November-2020.pdf), a temporary exceptional-policy comparison for enrollment while a prerequisite result was pending; it does not create a permanent Philippine or Servitech rule.
...
Draft and submitted-but-unreleased grades never affect registration. Clinic 4's narrow `AuthorizedPrerequisiteException` may temporarily permit one named dependent course while the prerequisite remains unreleased, but it never changes this projection to `Satisfied` and never contributes a grade, credit, units, GWA, or completion effect.

Every transition from `Pending` to a released or otherwise authoritative projection—including initial release, INC resolution, correction, or exact approved-credit mapping—recalculates the curriculum/requisite facts and routes affected active Registration Cases by exact version. `Satisfied`, or `ApprovedCredit` that exactly satisfies the named prerequisite, resolves a current exception without changing enrollment. `Unsatisfied`, `Incomplete`, or `WithdrawnOrDropped` ends the exception: a not-yet-finalized proposal/placement becomes stale, while an Official Enrollment remains intact pending Registrar review and any separately authorized Clinic 4 adjustment or Course Drop. A failed result alone does not automatically impose probation, dismissal, a reduced load, or an enrollment mutation.
...
- initial release, INC resolution, and correction producing the same idempotent active-case impact review, including a narrow pending-prerequisite exception with satisfied, adverse, and expired outcomes;
...
- one current pending-prerequisite exception resolved by a satisfying release, one adverse release before finalization, one adverse release after Official Enrollment while Adjustment is open, and one post-closure case with and without late-adjustment authority;
...
16. For an active next-Term Registration Case, release a previously pending prerequisite result. Prove: satisfying release resolves only the exception; adverse release before finalization invalidates proposal/placement; adverse release after finalization opens one review; open Adjustment uses the guarded change path; closed Adjustment requires exact late authority; absence of authority preserves enrollment/COR and names the next path; duplicate events, failed email, stale finance, capacity conflict, and retry never create a second review or partial mutation.
...
| Released-result active-case impact | TALA derives from an initial release, INC resolution, or correction; Registrar owns any Clinic 4 action | Exact result/event, prerequisite/curriculum versions, active Registration Case/course, exception/expiry, and pre/post-finalization state | Student/Registrar see source, affected course, owner, and next action; no grade value appears in mail and no new navigation/workflow aggregate is created | Duplicate/stale delivery creates no second review. Pre-finalization facts become stale when invalid; Official Enrollment remains until an authorized guarded change |
