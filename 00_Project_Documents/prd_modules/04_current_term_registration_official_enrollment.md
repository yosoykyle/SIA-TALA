...
- Individually Advised continuing-Student cases with one exact institutionally authorized prerequisite exception while the prerequisite result remains unreleased.
...
| Narrow prerequisite exception current | Record one `AuthorizedPrerequisiteException` for an Individually Advised continuing-Student case | Registrar records an externally authorized result | Exact authority office/reference/date, reason, source and consuming Terms, prerequisite registration/result, dependent course/Class Offering, permission scope, effective period/expiry | Prerequisite result is still unreleased; exact case/course/class current; no unrelated permission; other academic/capacity/schedule/load/finance checks still pass | Append-only exception evidence permits only the named proposal, placement, and finalization while current | Result release or expiry resolves/ends the permission; predecessor and learner-confirmed proposal remain historical | Learner sees the source, affected course, owner, expiry, and consequence; no `P`, satisfaction, credit, units, GWA, or learner classification appears |
...
|---|---|---|---|---|---|---|
| Student eligibility | Clinic 2 Ready Applicant or Clinic 5 released result/lifecycle/progress effect, plus a current exact `AuthorizedPrerequisiteException` only when applicable | Registrar consumes; Clinics 2/5 own sources; Registrar records the exception | Applicant is ready or Student has `Allowed`/authorized `AdvisingRequired`; no unresolved block/decision; any pending-prerequisite permission is current and exact | Case creation is blocked when entry eligibility fails; placement is blocked when proposal eligibility fails; finalization is blocked when the current effect becomes `Blocked`/`PendingDecision` or an exception ends | Start case; prepare proposal; finalize | Correct the owning source, record the exact external authority, or revise the proposal; never create a local/general override |
...
| Valid class placement | Clinic 3 published classes plus reservations, requisites, conflicts, capacity, and any exact current prerequisite exception | Registrar | Every proposed recurring course has a valid protected placement or approved no-meeting treatment; every requisite is satisfied or the one named pending-result permission is current | Finalization blocked; expired/adverse permission makes the proposal/placement stale; shortage owner/deadline shown | Finalize official enrollment | Resolve shortage, revise the proposal, record exact authority, or revise timetable through Clinic 3; no general requisite bypass |
...
- Pending and incomplete results do not satisfy prerequisites. Only the exact temporary permission in Section 4.4 may allow one named dependent course while its prerequisite result remains unreleased.
...
### 4.4 Authorized prerequisite exception

`AuthorizedPrerequisiteException` is an append-only authority record for one Individually Advised continuing-Student Registration Case while one prerequisite result is still unreleased. It records:

- Student and Registration Case, source Term, consuming Term, prerequisite Course and official registration/result reference, dependent Course and exact Class Offering;
- authority office, reference, decision date, reason, exact permitted proposal/placement/finalization scope, effective period, expiry, Registrar recorder, recorded-at time, and predecessor/supersession when corrected; and
- the learner-safe explanation, responsible office, consequence at result release or expiry, and current source versions.

It temporarily permits only the named dependent course through the existing proposal, placement, learner-confirmation, capacity, schedule, load, finance, and finalization checks. It creates no `P`, released grade, course satisfaction, approved credit, attempted/earned units, GWA input, curriculum completion, permanent learner classification, unrelated requisite permission, or general override action.

The permission ends at the first released or otherwise authoritative course-result projection, or at its recorded expiry:

- `Satisfied`, or an exact `ApprovedCredit` mapping that satisfies the named prerequisite, resolves it without changing the proposal or Official Enrollment.
- `Unsatisfied`, `Incomplete`, or `WithdrawnOrDropped` ends it. Before finalization, the current proposal/placement becomes stale and finalization is blocked until Registrar issues a valid successor proposal. After finalization, the existing registration and COR remain intact while an academic-result impact review opens.
- Still `Pending` at expiry ends it with the same pre/post-finalization handling; TALA never silently extends the authority.
- Duplicate or stale result/expiry processing creates no second review or mutation. A corrected authority appends a successor and revalidates the exact case.

The [UP November 2020 memorandum](https://publicservice.up.edu.ph/wp-content/uploads/2020/11/OVPAA-MEMO-2020-139_15-November-2020.pdf) is a temporary, exceptional Philippine comparison showing that pending-prerequisite progression can require a bounded institutional rule. It does not establish Servitech policy or a permanent learner entitlement.
...
| AuthorizedPrerequisiteException | Record one exact temporary permission while a prerequisite result is unreleased | Institution owns decision; Registrar records it | Immutable version or event | Proposal, placement, finalization, result-impact review, learner explanation | Separate authority evidence because expiry/result release must be reproducible; not a grade, satisfaction result, learner type, or generic override engine |
...
- Pending prerequisite with a current exact exception, satisfied/adverse/expired outcomes before and after finalization, and no released `P` or automatic enrollment mutation.
...
| `REG-2026-0011` | Individually Advised continuing Student with one unreleased prerequisite and exact `AuthorizedPrerequisiteException` | No `P`/satisfaction effect, exact authority/expiry, learner-confirmed proposal, later satisfying release resolves only the permission |
| `REG-2026-0012` | Adverse prerequisite release after Official Enrollment | One idempotent academic-result review; open-Adjustment guarded outcome plus closed-Adjustment branches with and without exact late authority; no automatic registration/COR mutation |
...
| Registrar/Student; `REG-2026-0011` | Ready to prepare → Enrollment | Record the exact prerequisite exception, issue/confirm the proposal, then release a satisfying prerequisite result | Authority, source/consuming Terms, courses/Class Offering, expiry, **This permission is not a grade or completed prerequisite** | Clinic 5 result resolves only the permission; case remains current | Confirmed proposal and exception history | Missing/stale/expired authority records nothing and cannot satisfy the prerequisite | No `P`, credit, units, GWA, learner type, or unrelated permission appears |
| Registrar/Student; `REG-2026-0012` | Adjustments and Drops / Academics → Enrollment | Process an adverse released result after Official Enrollment, then exercise open and closed Adjustment branches | One result-impact review, current COR, owner, next action, capacity/schedule/Finance state | Applied authorized change synchronizes Faculty/Accounting/Student; no-authority branch changes nothing | Successor COR only after an applied guarded change | Duplicate result, mail failure, capacity conflict, stale Finance, or closed window without exact late authority creates no duplicate/partial mutation | Current enrollment remains official until one authorized transaction commits |
...
| Authorized prerequisite exception and later-result review | Institution authorizes one exact pending-result permission; Registrar records/reviews | Individually Advised continuing-Student case; unreleased prerequisite; exact courses/Class Offering/Terms/authority/expiry; all other checks current | **Record prerequisite exception** shows exact permission, expiry, learner explanation, and no-grade/no-satisfaction boundary; learner confirms the resulting proposal | Satisfied release resolves only the permission. Adverse/expired source invalidates pre-finalization facts or opens one post-finalization review. No duplicate review, silent enrollment change, or automatic extension |
