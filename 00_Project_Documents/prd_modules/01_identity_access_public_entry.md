...
| Queue, mail transport, and official sender | Operational integration readiness | Deployment operator/System Administrator | Verification, invitation, and recovery messages can be dispatched and outcomes recorded | New self-registration and Staff invitation blocked | Create/verify account; invite/activate Staff | Restore transport/sender and use authorized idempotent resend |
| Official support | Approved institutional support contact/path | Institution/System Administrator | A monitored contact or support path is published | Disabled/recovery guidance blocked | Render public/auth/failure recovery | Publish the approved monitored support path |
| Privacy and accessibility information | Approved institutional pages or external references | Institution/System Administrator | Both references are available and reachable | Public registration blocked | Register Applicant account | Publish/correct the approved references |
...
Readiness is derived and failed-first. Passed checks remain collapsed. A runtime mail failure after an account transaction does not erase the account or reverse another institutional transaction; it records failure, presents a safe retry/support path, and allows an authorized resend.
