...
- automated backup-job results recorded by TALA or its approved local job boundary, including the latest locally verified off-host generation and whether the configured production target is overdue.

Every row shows the plain user label `Available`, `Needs attention`, `Unavailable`, or `Not recently checked`, together with the evidence source, as-of time, responsible owner, and safe next step. Existing internal status values may remain implementation detail, but routine copy must not expose architecture terminology. A missing or unknown result never appears healthy. Provider dashboard status, primary-host backup state, independent backup-provider state, and any physical-copy/custody state say that no recent approved evidence is available unless verified external evidence is deliberately imported through a later approved authority. The precise evidence boundary remains available under technical details.
...
Production recovery is an external operational responsibility, not a Clinic 6 product workflow or capstone browser journey. The product contract is deliberately provider-neutral:
...
- Prospective production requires an independently controlled, client-side encrypted off-host copy outside the primary hosting failure and owner-account boundary, generated from a transaction-consistent export with integrity and application-version evidence.
- A named Infrastructure Custodian must monitor the configured backup objective and prove restoration in isolation before production acceptance. The institution's privacy/DPO authority owns provider, processor/transfer, retention, key-custody, removable-media, and lawful-disposal decisions.
- Exact provider, tool, cadence, retry, retention, RPO/RTO, offline-media, and restoration order belong to the Architecture Specification and deployment runbook. Cloudflare R2, restic, and the client-owned ORICO enclosure are evaluated production candidates, not mandatory TALA features or capstone acceptance dependencies.
- TALA exposes no provider, shell, media, or restore controls and may display only safe locally recorded outcome, as-of, overdue, and next-action evidence. Provider and physical-media facts use plain not-recently-checked guidance and never appear healthy without separately imported approved evidence; technical details retain the exact source and verification boundary.
...
| System Health | System Administrator distinguishes current evidence, action-needed evidence, and facts not recently checked | Plain service summary, evidence time/source, responsible owner, safe next step, then technical details | Status/filter/search; local refresh; optional self-test email only |
| Governance & Audit | Authorized System Administrator investigates high-value evidence | Selected tab, filters, newest events, plain records-procedure guidance, then technical details | Actor/type/date/search; read-only detail; no manual attestation or compliance verdict |
...
- Print views use practical 12 mm margins and are A4 portrait unless an owning PRD expressly selects landscape. They are monochrome-safe, repeat identity and table headings, add automatic page numbers when multiple pages are produced, avoid clipped rows, retain status/disclaimers, and do not depend on navigation, remote fonts, or background color. CSVs remain downloadable data rather than printable documents.
...
| System assurance | Plain status and next action with exact locally recorded evidence and technical boundary on disclosure | Provider operations console; manual attestation checklist | Prevents unsupported claims and dangerous infrastructure controls without exposing architecture wording as routine copy |
...
| 18 | System Administrator | Open System Health | Plain summaries distinguish current local evidence from facts that were not recently checked; technical details retain source and boundary | Missing/unknown evidence never appears green or available |
| 19 | System Administrator | Open Governance & Audit | **Automatic record disposal is not available in TALA. Follow the institution's approved privacy and records procedure.** Technical details retain the exact product boundary | No attestation, policy-approval status, or disposal action appears |
...
| System Health/Governance | System Administrator read-only | Locally evidenced fact and as-of time only | No consequential provider/control confirmation exists | Missing evidence is shown as `Not recently checked`, never healthy, with the exact source/boundary in technical details. No arbitrary-recipient email, test charge, solver run, backup, restore, command, or attestation action |
| External backup/recovery evidence | Named Infrastructure Custodian operates outside TALA; privacy/DPO authority owns provider, processor/transfer, retention, key, media, and disposal approval | Configured production objective, scoped credentials, valid manifest/integrity result, current retained generation, and isolated restore evidence | TALA records only safe local outcome/as-of/overdue/next-action evidence; no secret, personal-data payload, provider control, or media command is exposed | Failed operations alert the custodian; invalid keys or media stop recovery; the deployment runbook selects the next valid source without silently mutating live records |
...
No Clinic 6 product decision remains gated. Automatic retention disposal is intentionally outside the MVP, while lawful retention schedules, privacy requests, legal holds, secure disposal, provider/processor/transfer approval, recovery-key custody, optional media rotation, and disaster restoration remain external institutional responsibilities. Completed/alumni records continue in the live system with the documented read-only access until an attributable lawful retention decision applies; no disaster-recovery copy forms an archive database. Accounting tax documents, cash movement, provider administration, refunds, and collection activity likewise remain external.

Production readiness is gated on institution-owned hosting and independent-backup accounts; named billing, Infrastructure Custodian, and privacy/DPO owners; approved processor and international-transfer safeguards; current provider pricing; controlled recovery-key custody; installed and monitored backup automation; current verified generations; and successful isolated restore evidence. The Architecture Specification may evaluate concrete providers and media, but procurement and operational proof do not create a capstone browser-acceptance or data-migration requirement and are never presented as already operating.
