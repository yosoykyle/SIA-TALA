...
- The [Data Privacy Act IRR](https://privacy.gov.ph/implementing-rules-regulations-data-privacy-act-2012/) and [NPC Circular No. 2023-06](https://privacy.gov.ph/wp-content/uploads/2024/05/2023-compendium-2.pdf) require proportionate security, controlled access, continuity, backup, restoration, remedial-time planning, periodic review/testing, processor safeguards, and institutionally governed retention/disposal. They prescribe no numeric recovery-point or recovery-time objective. TALA's six-hour RPO and eight-hour RTO are project-authorized prospective engineering targets, not statutory, CHED, Servitech, SLA, or achieved-capability claims. They do not require TALA to provide an automatic disposal engine; the MVP keeps those compliance operations external.
- The supplied current-workflow evidence records an end-of-business-day Google Drive copy of the manual finance ledger. That is scoped practice evidence for routine backup only; it neither establishes a six-hour requirement nor approves Google Drive as TALA's production backup provider.
...
- automated backup-job results recorded by TALA or its approved local job boundary, including the latest locally verified off-provider generation and whether the six-hour target is overdue.
...
Active and `Completed` Student, enrollment, grade, academic-history, account, and official-output records remain in the live authoritative system until an attributable institutional retention decision lawfully directs otherwise. Completion preserves the existing read-only historical Student access; a backup never removes, archives, or replaces a live record. Disaster-recovery generations are unavailable for ordinary browsing, reporting, or record retrieval and do not extend an approved lawful-retention period. When a restore uses a generation created before an externally completed lawful disposition, the Infrastructure Custodian must reconcile that disposition before restored service reopens so disposed material is not silently republished.

#### Deployment and recovery authority

The following design is a **project-authorized prospective production baseline**. It is not a law- or Servitech-mandated policy, SLA, implemented control, or achieved recovery claim:

- The live MySQL database and private application storage remain the sole authoritative operational source. A backup is a time-bounded disaster-recovery copy, not an archive database or alternate TALA record store.
- The selected host direction remains a self-managed [Hostinger KVM 1 VPS](https://www.hostinger.com/ph/vps-hosting), subject to procurement-time specification and price recheck. Hostinger's included weekly VPS backups and controlled pre-change snapshots are supplemental full-host recovery points only.
- At least every six hours, one non-overlapping external operating-system job creates a transaction-consistent export of authoritative application data, required private documents and reproducible official-output source files, plus a manifest with backup time, application revision, migration state, and integrity evidence. Authoritative audit, payment, webhook, email-idempotency, and correction evidence are included; rebuildable cache/session state, temporary exports, replaceable build files, Git-held source, and recovery secrets are excluded or rebuilt.
- The export is encrypted client-side and written through a restic-compatible S3 repository to a private institution-owned Cloudflare R2 Standard bucket outside the Hostinger failure and owner-account boundary. R2 provider-side encryption is supplemental. The institution must approve processor terms, international-transfer safeguards, billing ownership, the best-effort `apac` location hint, and the current checkout price before production use.
- Retention keeps every six-hour off-provider snapshot for seven days, one daily snapshot for 30 days, and one monthly snapshot for 12 months. Expiration of these disaster-recovery generations is not disposal of the corresponding live institutional record.
- One failed scheduled run retries once after 30 minutes. A failed retry alerts the named Infrastructure Custodian and repeats the alert daily until a successful generation is recorded. The local status becomes overdue when the latest verified off-provider generation is older than six hours. Repository integrity is checked weekly.
- Once per week, the Infrastructure Custodian copies the latest verified off-provider repository through an authorized recovery workstation to alternating independently encrypted ORICO Drive A and Drive B media. At least one verified drive remains safely unmounted, disconnected, and separately stored. ORICO never connects to ordinary TALA screens or becomes a live/archive store.
- An isolated end-to-end restore is recorded at least quarterly, alternating R2, Drive A, and Drive B so every source is proven within one year. Measured loss and restoration duration are retained as operational evidence.

The **RPO target is six hours of authoritative changes**. The **RTO target is eight elapsed hours** after recovery is formally declared and the necessary accounts, keys, media, infrastructure, and personnel are available, ending when core authenticated SIS service with verified current authoritative data is restored. External integrations may remain explicitly degraded. Neither target becomes achieved until a restore test proves it.

The institution remains the Personal Information Controller and owns the Hostinger, R2, domain, billing, and recovery accounts. Its privacy/DPO authority approves processor and transfer safeguards, retention, removable-media handling, and lawful disposal. A named Infrastructure Custodian operates monitoring, media rotation, recovery, and escalation. The VPS receives only a bucket-scoped R2 credential; the restic recovery password remains outside Hostinger and R2, with two separately controlled institutional recovery copies. Developers may configure and document the process during deployment but must not retain sole owner credentials after handover.

Recovery provisions or validates clean infrastructure, uses the newest valid R2 generation for authoritative data, may use a Hostinger provider backup to accelerate infrastructure restoration before reconciling newer independent data, and falls back to the latest ORICO copy when Hostinger or R2 access/integrity is unavailable. Restoration occurs in isolation first. Before authorized cutover, the custodian verifies the manifest, database, private files, authentication, and critical journeys; invalidates restored sessions; clears rebuildable caches; reconciles queued/integration work idempotently; and reapplies any externally recorded lawful disposition made after the selected generation. These are external operational responsibilities. TALA exposes no provider, shell, media, or restore controls and may display only locally recorded evidence.
...
| 21 | Infrastructure Custodian and System Administrator | Complete one six-hour off-provider job, then exercise a failed run and its 30-minute retry | Success records generation/as-of evidence; final failure alerts the custodian; the existing System Health projection becomes overdue after six hours | No overlapping run, false healthy state, provider control, or new backup dashboard appears |
| 22 | Infrastructure Custodian | Restore the newest valid R2 generation into isolation | Manifest, authoritative data, private files, authentication, critical journeys, measured loss, and measured duration are verified before cutover | Invalid generation or check failure never replaces live service; next valid generation or fallback is selected |
| 23 | Infrastructure Custodian | Use a Hostinger backup to accelerate clean-infrastructure restoration, then reconcile the newer R2 data generation | Provider restore remains supplemental and current authoritative data comes from the verified independent generation | Whole-VPS overwrite is never treated as the only copy or proof of the six-hour target |
| 24 | Infrastructure Custodian | Simulate unavailable or invalid R2 access and restore from the newest valid ORICO copy | Offline repository restores without making ORICO a live/archive store | Failed media is quarantined; the alternate verified drive remains disconnected until selected |
| 25 | Infrastructure Custodian | Attempt restore without a valid repository password or scoped provider access | Recovery stops safely and invokes the separately controlled institutional recovery copy/owner | No credential is exposed through TALA, logs, provider status, or the System Health page |
| 26 | Infrastructure Custodian | Perform the quarterly isolated restore rotation across R2, Drive A, and Drive B | Every source is proven within one year and measured RPO/RTO evidence is retained | An unproved or slower restore remains a target failure with owner and remediation, never an achieved claim |
| 27 | Infrastructure Custodian and privacy/DPO authority | Restore a generation predating an externally completed lawful disposition | External disposition evidence is reconciled before restored service reopens | Disposed material is not silently republished; no TALA disposal or archive workflow is introduced |
...
| External backup/recovery evidence | Named Infrastructure Custodian operates outside TALA; privacy/DPO authority owns processor, retention, media, and disposal approval | Non-overlapping six-hour generation, one 30-minute retry, scoped credentials, valid manifest/integrity result, current retained generation, and quarterly isolated restore evidence | TALA records only safe local outcome/as-of/next-action evidence; no secret, personal-data payload, provider control, or media command is exposed | Failed retry alerts the custodian and becomes overdue after six hours; invalid keys/media stop recovery; restore falls through the documented R2, provider-assisted, and ORICO order without silently mutating live records |
...
No Clinic 6 product decision remains gated. Automatic retention disposal is intentionally outside the MVP, while lawful retention schedules, privacy requests, legal holds, secure disposal, processor/transfer approval, recovery-key custody, media rotation, and disaster restoration remain external institutional responsibilities. Completed/alumni records continue in the live system with the documented read-only access until an attributable lawful retention decision applies; neither off-provider generations nor ORICO media form an archive database. Accounting tax documents, cash movement, provider administration, refunds, and collection activity likewise remain external.

Production readiness is gated on institution-owned Hostinger and Cloudflare R2 accounts; named billing, Infrastructure Custodian, and privacy/DPO owners; approved processor and international-transfer safeguards; current provider pricing; two separately controlled recovery-password copies; installed and monitored backup automation; verified six-hour generations; weekly alternating ORICO custody; and quarterly isolated restore evidence. These gates do not create a capstone data-migration requirement or claim that the prospective controls are already operating.
