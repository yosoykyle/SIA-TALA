...
Every row shows `Available`, `Attention`, `Unavailable`, or `Unknown`, evidence, as-of time, and safe next step. `Unknown` never appears healthy. Provider dashboard status, primary-host backup state, independent backup-provider state, and any physical-copy/custody state display `Not checked by TALA` unless verified external evidence is deliberately imported through a later approved authority.
...
- TALA exposes no provider, shell, media, or restore controls and may display only safe locally recorded outcome, as-of, overdue, and next-action evidence. Provider and physical-media facts remain `Not checked by TALA` without separately imported approved evidence.
