...
- The prospective production infrastructure direction is a self-managed Hostinger KVM 2 VPS with an independent encrypted off-host backup and project planning targets of six-hour RPO and eight-hour RTO. The exact backup provider, optional offline-media layer, procurement, and recovery procedure are deployment decisions; Cloudflare R2 and the client-owned ORICO enclosure remain evaluated candidates rather than MVP product requirements. Provider facts and recovery performance require external operational evidence and are not capstone journey-acceptance prerequisites.
