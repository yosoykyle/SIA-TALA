...
- **Recorded external decisions:** overload, late adjustment, transfer credit, discipline, shifting, and financial accommodations
...
| Clinic 5 | Grade-roster action/return/release, INC action/deadline and resolution/lapse, correction, progress/lifecycle, completion action, conferral | Owning roster/result/decision/conferral reference; grade values and attachments are excluded |
...
- **Accepted:** Official enrollment requires clearance for the amount currently required, not a universal zero balance. Funding arrangements and later obligations are separate from academic finalization; service-specific restrictions cannot globally block login or examinations.
...
- **Accepted:** GWA is the full-precision, unit-weighted mean of released numeric attempts. All attempts count, while PE and NSTP—including CWTS, LTS, and ROTC equivalents—are excluded under the client-confirmed Servitech rule. `INC`, dropped or withdrawn results, and nonnumeric approved credit are excluded. GWA is displayed to two decimals in academic views and is not printed on the standard TOR.
- **Accepted:** An unresolved GWA-eligible `INC` withholds the current term and current cumulative GWA. It receives a one-academic-year deadline from the original term end, does not satisfy prerequisites, and lapses to `5.00` if unresolved. Resolution or lapse preserves history and recalculates every affected projection.
- **Accepted:** Authorized grade corrections append a superseding result without a hard technical cutoff. They recalculate GWA, curriculum evaluation, progress recommendations, and completion readiness, while earlier decisions and issued transcript snapshots remain historical.
...
- **Accepted:** Student Academics presents released grades, GWA or its explicit pending state, curriculum evaluation, confirmed progress, units, completion readiness, and history. Students may print an unofficial record only.
...
- TALA provides one narrow Program-and-Term Fee Plan and continuous same-human-subject/RegistrationCase Term Account companion. `Person` is only a documentation label for identity continuity, not a new master record. TALA does not replace Accounting's bookkeeping, cashiering, collections, general ledger, refund, or BIR-invoicing procedures.
- Clinic 6 publishes `EnrollmentPaymentRequirementProjection` to Clinic 4. It states the assessment, current enrollment obligation, verified/covered amount, remaining required amount, state/basis, source/as-of time, later-obligation indicator, and authorized account link. `Cleared` never means a lifetime zero balance.
...
| Clinic 5 | `OfficialCourseResultProjection`, `AcademicEnrollmentEffect`, `CurriculumEvaluation`, `CompletionReadiness`, `TranscriptSnapshot` |
| Clinic 6 | `FeePlan`, `TermAccount`, `AssessmentVersion`, `PaymentEvidence`, `PaymentPosting`, `EnrollmentPaymentRequirementProjection`, `OfficialOutputPaymentClearance` |
...
| Clinic 6 → Clinic 4 | `EnrollmentPaymentRequirementProjection` | Term Account/Assessment version and as-of time; `Unavailable` blocks finalization | Recalculate finance, require lifetime zero balance, or apply a global hold |
...
| `Quarantine` | Existing fields, services, pages, actions, routes, and tests remain untouched until post-authority task derivation maps every consumer and proves safe migration. |
...
| `Replace` | Generic Fee Rule precedence, silent 20% fallback, Enrollment/StudentProfile-only account ownership, immediate manual confirmation, fragmented finance UI, and legacy host selection. |
...
| Continuing and advised enrollment | Standard and Individually Advised proposals, prerequisites, shortages, reservations, timetable revision | Clinic 3 owns one revision event/email; no arbitrary course shopping or silent learner move |
| Grade release and correction | Designated Faculty roster, returned rows, complete release, INC, correction, RegistrationCase review | Only released results cross clinics; no overwritten result or silent registration change |
...
| Account and payment | Fee Plan readiness, manual evidence, exact-due checkout, under/mismatch, duplicate and missing/late webhook, reversal | Browser return never posts; one posting and one email; late event matches idempotently; no global hold |
...
| **5 — Teaching and Official Academic Record** | PRD 05 | Official rosters, final grades, release, correction, INC, GWA, curriculum evaluation, progress, lifecycle, transcript, and completion | Approved; complete-set review passed |
| **6 — Accounts and Operations** | PRD 06 | Fee Plans, continuous Term Accounts, payment evidence, bounded enrollment/output-clearance projections, non-tax account outputs, contextual exports, System Health, privacy, audit, recovery, and assurance | Approved; complete-set review passed |
...
- The current semestral structure and optional special/summer term remain provisional until supported by an approved calendar.
