...
| Student Hub and applicant/student self-service | Student-facing read-only visibility for profile, enrollment status, COR, schedule, grades, balance, notifications, and FAQ/help, with mutations added only through approved dedicated services. | Campus self-service benchmark; Livewire/TallStackUI/PWA pattern. | Core-lite |
| COR, SOA/payment evidence, and QR verification | COR is generated only from canonical enrolled data. SOA/payment acknowledgement artifacts are generated only from Accounting-owned ledger/payment evidence. TOR, Form 137, diploma, and full credential issuance are outside active TALA scope; Form 137/Form 138 remain prior-education admission evidence only. | Enrollment-verification, student-financials, PDF/QR verification baseline. | COR and finance evidence Core; full credential issuance outside scope |
...
| **Student**                   | Official enrolled student        | View grades, schedule, COR, financials, notifications, and approved service-request status    |
...
- TOR, Form 137, report cards, diplomas, certificates, and full credential issuance are outside active TALA scope. Their source records may exist as admission evidence, grade history, or completion status, but TALA does not generate, request, fulfill, or release those credentials.
...
4. **Finalize**: Registrar verification atomically finalizes the verified package into official academic history. Only verified/finalized grades are released to the Student Hub and prerequisite/progression services. Transcript, report-card, and full credential generation remain outside active TALA scope.
