...
**Post-Commit Changes**: Availability changes after schedules are committed do not automatically affect approved schedules. Any official schedule change requires Registrar approval, reason capture, and audit history containing the old values and new values.
...
3. **Manual Adjustments**: Accounting may add "One-Off" charges or overrides to a student's ledger if needed.
...
**Current TAL-12 Implementation Scope Note**: COR verification/control surfaces are part of the current admin hardening scope. A full COR template/layout editor is not yet counted as completed Pre-UAT evidence unless a separate implementation item builds and tests it. Until then, COR template editing remains a tracked System Super Admin capability target, not an available admin workflow.

#### 8.3.2 Staff Account Creation Boundary

System Super Admin may create and maintain **staff accounts only** for Registrar, Accounting/Cashier, Faculty, Academic Head, and System Super Admin users. The staff-account form captures First Name, Middle Name (optional), Last Name, Suffix (optional), username, email, password, active/inactive status, and exactly one approved staff role. Student/applicant accounts are created by applicant intake, official handover, or Registrar walk-in intake, not by the generic staff user-management form. Archive fields, verification timestamps, and audit fields are system-managed and must not be exposed as ordinary creation inputs.
...
- **Trigger**: **System Super Admin** clicks "Archive Account" and uploads the official resignation/termination letter
...
**Current TAL-12 Implementation Scope Note**: The implemented Filament surface is **Import Batch Audit**. Generic create/edit routes and actions are disabled. Existing pending batches may be viewed and state-transitioned through approved commit/cancel controls, but dedicated upload, parse, preview, and error-report pages remain a separate import implementation item and must not be treated as complete Pre-UAT evidence.
...
- **Manual Courier Details**: After shipping, the Registrar records the courier name, tracking number or `N/A`, actual shipping fee, and required courier receipt photo.
- **Courier Data Rules**: Shipping fee is stored as a two-decimal money value, receipt proof is stored privately, and tracking `N/A` is normalized to uppercase so student notifications and ledger posting stay consistent.
