...
| Finance, assessment, ledger, payments, SOA, and payment evidence | Fee assessment, discounts, installments, manual/online payment confirmation, immutable ledger, computed balance/clearance, SOA/payment evidence, externally issued receipt/reference recording, and privacy-safe finance visibility. | SIS Student Financials benchmark; PayMongo payment lifecycle; SIA SOA/payment evidence. | Core |
...
| Official generated documents | COR, academic records, SOA/payment evidence, and completion credentials are generated only by their authoritative enrollment, grade, finance, or completion workflows with issuance and verification evidence. Form 137/Form 138 remain prior-education admission evidence. | Student Records transcript/enrollment-verification/graduation benchmark; PDF/QR verification baseline. | COR Core; other artifacts owned by their source workflow |
...
| Official generated documents and QR verification | COR is available only from canonical enrolled data. Academic, finance, and completion artifacts derive from their authoritative enrollment, grade, curriculum, ledger, or completion records. Issuance records preserve type, subject, source snapshot, template version, issuer, issue time, reference/serial, checksum, state, and authorized release evidence. QR verification uses an opaque token or signed route and reveals only minimal validity metadata. | Missing eligibility, holds, unfinalized grades, invalid enrollment, revoked/superseded issuance, or unauthorized access blocks generation/release. Corrections create superseding or revoking history; generated files never become the operational source of truth. | COR Phase 1; other artifacts follow their owning workflow; BM-13, BM-14, BM-15; official-document and QR verification cases. |
...
| **Financial and transaction evidence** | Payment proof, promissory-note attachment, externally issued receipt/reference image | Preserve privately and link immutably to the payment, note, or accounting event. OCR may suggest references/amounts but never confirms payment, approval, or settlement. |
| **Generated official artifacts** | COR, report card, assessment, SOA/payment acknowledgement, completion credentials | Store an immutable issuance snapshot or reproducible PDF artifact with document type, subject, term/request, template version, issuer, issue time, checksum, verification token where applicable, and lifecycle state. Corrections supersede or revoke prior artifacts; they do not overwrite issued history. |
...
- Academic and completion artifacts are issued only through their authoritative Registrar, grade, transfer, or completion workflows. They are not created through a document-request portal.
- Diploma preparation/release remains a graduation/credential workflow; the first acceptance baseline may record eligibility/release evidence without automating outside-office credential processing or final diploma printing.
...
**Ledger Admin Surface Boundary**: The Ledger Entries screen is an Accounting review/evidence surface. Accounting may view and filter assessed fees, payments, discounts, penalties, credits, and balances, but must not create, edit, or delete arbitrary ledger rows through generic Filament CRUD. Ledger mutations are produced by assessment, discount, payment-confirmation, webhook, installment-penalty, refund, or approved adjustment services.
...
- **Drop-out Fee**: If a student officially drops out, an effective-dated withdrawal/refund fee is assessed based on approved institutional policy and applicable withdrawal/refund rules. Students may drop even with outstanding balances; any record-release hold must follow an approved policy.
...
- **Scope Contract**: Installment policies use the same canonical College program/year scope as fee templates. Only one active installment policy may exist for a scope; inactive historical policies may share a scope for audit and replacement history.
...
**Business Rule**: Every payment-related cancellation, refund, or adjustment is resolved by a versioned financial-disposition policy scoped to the deployment, effective period, term/program when needed, and typed event cause. The current approved workflow allows refund review of the admission/enrollment fee within 15 days of the externally issued receipt/payment-reference date and makes tuition non-refundable after official enrollment. These rules must not be hard-coded as universal TALA behavior; other deployments may use legally and operationally approved percentage schedules, account credits, term transfers, or controlled refund workflows.
