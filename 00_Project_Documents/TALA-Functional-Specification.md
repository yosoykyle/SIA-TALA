...
**Specification Scope Boundary:** This FS is intended to be the capstone/paper baseline for the final-form TALA system. It is grounded in SIA business evidence and feature-level benchmark review. Mature SIS references establish the normal lifecycle spine: person/student master data, admissions, enrollment, academic records, scheduling/classes, grades, student financials, self-service, generated artifacts, and completion/graduation processing. SIA-specific policy and workflow evidence narrows how that spine behaves locally.

**College-Only Scope Boundary:** The active TALA deployment covers College operations only. Prior-school credentials such as Grade 12, Form 138, Form 137, Good Moral, PSA, and similar records may appear only as admission evidence for College applicants.

**System-Owned Scope Boundary:** TALA models only system-owned SIS workflows: applicant intake, admission evidence review, enrollment, scheduling, student accounting evidence, grades, Student Hub visibility, roster exports, generated artifacts, privacy, and audit. Manual outside-office processes are not modeled as queues, statuses, permissions, routes, integrations, or acceptance scenarios.

**Payment Evidence Boundary:** TALA records student-account ledger evidence, payment references, and internal SOA/payment acknowledgements. Official tax documents and outside-office receipts remain outside the product baseline.
...
| Calendar, curriculum, subjects, sections, and scheduling | College academic years/terms, curricula, sections/delivery groups, faculty availability, Registrar-owned subject/faculty assignment from curriculum demand, conflict validation, and solver-assisted schedule generation with staff review. | Mature SIS course/schedule/class-readiness model; OR-Tools CP-SAT benchmark; SIA modality evidence. | Core/Supporting |
| Finance, assessment, ledger, payments, SOA, and payment evidence | Fee assessment, approved discount rules, manual/online payment confirmation, immutable ledger, computed balance/clearance, SOA/payment evidence, externally issued receipt/reference evidence recording, and privacy-safe finance visibility. Installments, promissory tracking, exam-access accommodations, and refund/disposition automation remain review candidates. | SIS Student Financials benchmark; PayMongo payment lifecycle; SIA SOA/payment evidence. | Core for assessment/payment/ledger/clearance; finance policy extensions review |
...
The system replaces fragmented manual processes (paper forms, Google Sheets, separate accounting logs) with a unified, automated platform. It streamlines the College student lifecycle-from online enrollment and document validation to grade management and financial clearance-while preserving applicable CHED, institutional-policy, prior-credential, and Data Privacy Act boundaries.
...
| Student Hub / PWA | Livewire + TallStackUI + Tailwind + Alpine.js, mobile-first tabs/cards/tables, read-only PWA cache for approved offline data | Active students can view dashboard, COR, schedule, grades, balance/payment status, applicable requirement or hold status, notifications, and FAQ/help. Document-request, credential-request, courier, and generic service-request status are not Student Hub features. Offline mode is read-only and must disable mutation actions. |
| Loading, splash, and offline fallback layer | Student Hub PWA via `erag/laravel-pwa`, manifest/service worker assets, TallStackUI buttons/alerts/progress, Livewire loading/offline directives, and Tailwind motion-safe/reduced-motion utilities | Installed Student Hub launches with branded manifest icon/splash behavior on supported platforms, shows stable loading/progress states for navigation and approved actions, disables duplicate submissions and offline mutations, provides a safe offline fallback, and respects reduced-motion/accessibility labels. |
...
| --- | --- | --- | --- |
| Enrollment, sectioning, finance clearance, inventory, and COR | Official handover occurs once after admission gates, readiness, finance, and capacity rules pass. It creates or reuses one canonical student profile, one term enrollment, secured placement, account access, and COR/class-list eligibility. Registrar can filter enrolled students by year, term, College program, year level, section, delivery group, and status and export an authorized generic roster. | Tentative placement grants no protected access and may expire. Missing readiness, unpaid required amount, full capacity, duplicate identity, or concurrent handover blocks atomically with an actionable reason. Institution-caused placement failure routes to Registrar resolution rather than applicant rejection. Manual outside-office work is not a TALA enrollment gate. | Phase 1 / Core; BM-01, BM-02, BM-04, BM-10, SIA-01; master enrollment/Registrar/COR cases. |
| Scheduling and automatic generation | Registrar prepares projected-demand sections/delivery groups and rooms while faculty submit term availability; Registrar then confirms eligible subject/faculty assignments. OR-Tools CP-SAT generates a reviewable day/time/room proposal from an immutable input snapshot. Registrar review and publication release the official schedule; students and faculty see only the published schedule. | Approved eligibility, submitted availability, configured maximum workload, calendar, capacity, assignment, room, faculty, and section conflicts are hard. Infeasible, unknown, timed-out, malformed, or hard-conflicted runs publish nothing and expose diagnostics. Published schedules are immutable and corrections require a superseding draft/version. | Phase 1 when schedule is on the demo path, otherwise Supporting; BM-05, BM-06, SIA-01; schedule generation and schedule publication cases. |
| Finance, assessment, payments, ledger, SOA, and payment evidence | Effective-dated fee, approved discount, scholarship, and downpayment policies produce an assessment. Manual Accounting confirmation and PayMongo outcomes post through the same idempotent immutable ledger contract. Balance and finance clearance are computed; SOA/payment acknowledgements are derived internal artifacts. | Duplicate references/webhooks post once; failed or pending provider outcomes do not clear finance; overpayment remains credit; corrections use reversal/adjustment entries rather than editing history. Balances remain private and debt does not become an undocumented exam-access block. Installments, promissory tracking, exam-access accommodation, and refund/disposition automation remain review candidates. | Phase 1 for assessment/payment/clearance; advanced finance-policy workflows review; BM-01, BM-05, BM-09, BM-13, SIA-01; finance and payment cases. |
...
| **Student**                   | Official enrolled student        | View grades, schedule, COR, financials, notifications, holds, and FAQ/help                    |
...
- Academic Head may view only read-only finance status and fee template/downpayment rules.
...
- Academic Head cannot process payments, create assessments, apply discounts, approve promissory notes if promoted later, mutate installment policies if promoted later, or edit finance records.
...
| Student authentication | Student Hub access requires an active user with the `student` role and an official student profile/enrollment context. | An active student can see only their own profile, enrollment status, COR, schedule, grades, balance summary, requirement/hold status, notifications, and help. |
...
**Target Audience**: College freshmen, transferees, returnees/readmission cases, and approved College applicant pathways.
...
| **Academic & Status**    | College Program/Course (IT/BM/THM/etc.), Year Level (1st–4th Year), Status (New/Transferee/Returning where approved), Assigned Section (System Generated/Pending) |
...
**College Program Check**:

- The system explicitly asks for the College program/course and applicant route.
- "Student Type:" (New Student / Transferee / Returning where approved)
- Applicant selects Program/Course and Year Level.
...
**Modality Selection**: Applicants may declare a preferred College learning mode:
...
Before applicant intake opens for a term, the Registrar must publish a College admission offering and activate a versioned requirement policy for each supported scope. A scope composes the term, College entry route, prior-credential pathway, citizenship/compliance profile, applicable program/year level, and only those learner-support attributes that lawfully change an admission requirement or accommodation. `Regular`, `transfer`, and `returning` are the approved College entry routes; `old curriculum` and ALS-related eligibility are prior-credential pathways only when approved for College admission; `foreign` is a citizenship/compliance profile; and IP or disability/SEN indicators are purpose-limited support attributes, not mutually exclusive applicant types. One applicant may therefore match more than one dimension.
...
**Initial Offering Profile**: The initial public/system acceptance offering set is `Regular College Freshman` and `College Transfer`. Old-curriculum status is a College prior-credential pathway resolved inside a compatible published offering, not a separate application pipeline. ALS is not published unless the institution approves an explicit College ALS/equivalency pathway. Foreign-student compliance is an independently composable profile, but it remains unpublished unless the institution confirms that it accepts foreign applicants for the scoped term/offering and the Registrar activates a compliant evidence policy. IP and disability/SEN are optional, purpose-limited support attributes and must not be represented as denial-producing applicant routes. Cross-enrollee intake is removed from the active TALA scope and must not be seeded, displayed, or retained as a dormant applicant offering.
...
The consolidated client workflow now supplies a College-only operational baseline. The system seeds College rows as draft, versioned requirement policies; it does not implement them as conditionals in application code. Before publication, Registrar must confirm the offering is accepted for the term and the policy must pass evidence-method and deadline validation. Non-College rows are historical evidence and are not seeded as active offerings.
...
| **Regular College Freshman** | Grade 12 Form 138 or equivalent prior-education evidence, PSA Birth Certificate, Good Moral, 2x2 photo | Diploma/Certificate of Graduation, entrance-exam result, ID photos | Initial public offering |
...
| **ALS / Equivalency College Pathway** | Institution-approved equivalency or completion evidence, PSA Birth Certificate, Good Moral, 2x2 photo | Any remaining official rating/completion record required by the active policy | Inactive until the institution approves a College pathway and evidence rule |
...
**College Offering Boundary**: Only College admission offerings are publishable in the active deployment. Earlier non-College offering rows are historical source evidence and are not selectable, publishable, or used as resolver fallbacks.

**ALS / Equivalency College Boundary**: ALS or equivalency evidence for College admission is inactive until the institution approves a College pathway and evidence rule. If approved later, it composes with a College offering and must identify the credential family, eligibility effect, and retention/follow-up records without creating a separate non-College route.
...
**Enrollment Readiness and Capacity Contract**: A term may collect inquiries or applicant submissions before every academic setup step is complete, but payment clearance, payment-backed capacity security, and official enrollment handover remain blocked until the term is enrollment-ready. Enrollment readiness requires an active calendar/enrollment window, published admission offering and requirement policy, approved admission-capacity plan, ready curriculum/subject-offering scope, planned sections and delivery groups, submitted faculty availability, Registrar-owned eligible subject/faculty assignments, and a Registrar-published official schedule. These readiness requirements have no schedule or availability bypass in the active baseline. A pre-payment section/schedule preview is tentative planning only; it does not reserve a protected seat, does not activate access, and expires at the earlier of the configured payment deadline, admission-window close, or manual Registrar cancellation for stale/ineligible cases. Confirmed minimum/full payment with immutable payment/reference evidence atomically secures capacity against every applicable capacity plan. If no compatible final section/delivery placement remains after payment through institution-caused delay or planning failure, TALA routes the case to `PendingInstitutionalPlacement`/Registrar resolution instead of silently rejecting or blaming the applicant.
...
- **Financial Policy**: Refund, withdrawal-fee, and financial-disposition automation remain review scope. All payment/ledger evidence remains immutable.
...
The latest approved workflow prepares a section and schedule before payment, while confirmed payment/reference evidence secures the slot and completes official enrollment. TALA must distinguish tentative pre-payment placement from a capacity-consuming secured seat. The locking, expiry, reassignment, and access transitions must be defined through benchmarked technical contracts before implementation.

**Step 6: Academic Pathing (The "Fork")**

**Path A: Freshmen (Auto-Sectioning)**

- **Logic**: System assigns a College section/delivery group according to program, year level, capacity, and schedule readiness.
...
- **If Failed Grades Detected**: Evaluate the active College progression policy and flag the appropriate retake, irregular, or probation outcome; do not infer a universal repeat-year outcome without an approved policy.
...
- **Scope**: The cap applies to regular promotion, College irregular self-selection, transferee credited-subject adjustments, and Registrar manual overrides.
...
- **Override Boundary**: Academic load overload for students remains blocked unless a separate approved policy exists. When a faculty maximum weekly workload is configured, exceeding it is a hard scheduling blocker. The Registrar must change the assignment or configured policy input and rerun readiness; no scheduling-role override bypasses it.
...
**Note**: Finance clearance is reached when Accounting confirms that the student has paid at least the required minimum downpayment, or has fully paid the assessed balance. It does not bypass an admission-gate requirement, but an approved retention-document undertaking is not an admission blocker. Promissory notes do not clear finance. The TALA enrollment state is changed only by TALA's audited enrollment workflow.
...
**Student Dashboard Backend Contract**: Before Student Hub UI buildout, the dashboard data source must be a read-only backend aggregate rather than hardcoded page content. The implemented contract provides student-owned profile/enrollment context, current schedule, financial balance and term summaries, finalized grades, dashboard holds, latest notifications, and published FAQ/help links. PWA/offline presentation may cache only these read-only outputs; approved payment entry remains online-only, and document-request, credential-request, courier, generic service-request, enrollment-mutation, and grade-correction mutation flows are not part of the active Student Hub baseline.
...
**Feature Boundary**: The Student Hub/PWA baseline is service-backed visibility first. Dashboard, schedule, grades, financials, COR, requirement/hold status, notices, and help must render only data owned by the authenticated active student and must hide unpublished schedules, draft grades, staff-only financial details, internal IDs, raw storage paths, and other students' records. Offline behavior is allowed only as a read-only convenience with clear freshness labels. Approved PayMongo payment entry must stay online and service-owned. Document upload/request, credential request, courier, and generic service request flows are removed from the active Student Hub scope; grade-correction and enrollment mutation UI remain review-only until separately promoted by feature audit.
...
Before the enrollment period opens, the Registrar prepares foundational data to enable automated evaluation and scheduling. System Super Admin does not configure academic schedule data because the role is read-only for academic operations. Academic Head authority remains limited to separately defined academic-review actions such as official grade corrections; it does not bypass scheduling hard constraints or publication ownership.
...
| **Scheduling Inputs & Drafts** | Section needs, curriculum-derived subject loads, room inventory, faculty availability, Registrar-selected subject/faculty assignment inputs, and proposed timetable drafts | Drives assisted conflict-free scheduling and final schedule export  |
| **Academic Calendar**          | College term name, start/end dates, scheduling dates, enrollment periods, and adjustment periods | Configures system state transitions and unlocks time-bound features |

**Note**: Without these datasets, the system cannot evaluate credits, assign sections, or generate schedules. Faculty availability and schedule generation stay locked until the target term has a name, `enrollment_starts_at`, `enrollment_ends_at`, and `scheduling_starts_at`.

#### 5.1.1 Academic Year and Term Timeline Setup for College

The system handles **College** academic calendars only. The Academic Year is the umbrella boundary; operational scheduling, enrollment gates, grading windows, billing periods, and availability collection are controlled by the configured College terms under that year.

- **College Timeline Handling**: College terms are subject to CHED rules, academic-hour requirements, and the school-approved/CHED-noted higher-education calendar. Terms are explicitly configurable and must be client-verified.
...
Each configured Academic Year supports multiple College operational terms (semesters and optional summer terms).
...
- **Cutover Policy (Approved)**: Calendar-model rollout activates at the next configured College semester boundary. Mid-term cutovers are not allowed.
...
| --- | --- | --- | --- |
| College | 1st Semester, 2nd Semester | `1st Sem AY 2026-2027`, `2nd Sem AY 2026-2027` | `term_start_date`, `term_end_date`, `class_start_date`, `class_end_date`, `enrollment_starts_at`, `enrollment_ends_at`, `late_enrollment_ends_at`, `credential_submission_cutoff`, `payment_deadline`, `adjustment_ends_at`, `scheduling_starts_at` |
| Optional Summer | Summer Term (when enabled) | `Summer AY 2026-2027` | Same required entries as College; only if summer term is opened |

**Calendar Setup Interface Rule**: School calendar setup is a two-level Admin Nexus workflow. First, authorized setup staff create or select an Academic Year. Second, authorized Registrar/Academic Head staff create child College Terms under that Academic Year. Terms, not Academic Years, hold the operational gates used by enrollment, scheduling, payment deadlines, adjustment locks, billing periods, faculty availability, class lists, and grade windows. The implementation uses a typed no-delete Academic Years admin resource plus the Terms admin resource; seed-only Academic Year setup is not acceptable for calendar operation.

**Required Cutover Entries:**
...
**Faculty Availability Cadence Rule**: Faculty availability is the only faculty-owned scheduling input and is submitted once per faculty per configured scheduling term, not once globally for the whole Academic Year. College normally requires one submission per semester, plus summer if a summer term is opened. Subject/faculty assignment remains Registrar-owned and uses curriculum-derived demand plus approved staff records. Post-deadline or post-lock changes use the controlled availability change-request workflow rather than a second normal submission.
...
| --- | --- | --- |
| `Program Code` | Yes | College program/course code such as `BSIT`, `BSTM`, `BSHM`, or another approved school code. |
| `Program Name` | Yes | Human-readable College program or course name. |
| `Curriculum Version` | Yes | Version label, e.g. `BSIT 2026`. |
| `Effective Year` | Yes | Four-digit starting year. |
| `Is Active` | Yes | Boolean-style input (`yes/no`, `true/false`, `1/0`). |
| `Year Level` | Yes | Must match canonical College values used by sections and enrollment, e.g. `1st Year`, `2nd Year`, `3rd Year`, `4th Year`. |
| `Curriculum Period` | Yes | `1st Semester`, `2nd Semester`, `Summer`, or another approved College period. |
...
| `Units` | Yes | Required College academic units. |
...
| `Academic Subject Type` | Yes | College academic meaning such as `general_education`, `professional_tesda`, or `program_specialization`. |
...
- **Program**: College program/course filter.
...
| **Modular print** | Delivery group | Not required | Teacher/adviser ownership required | No recurring class meeting conflict; approved assignment still required | No in-system pickup/submission scheduling in MVP |

**Delivery Pattern Logic**: Delivery Patterns are reusable, versioned rule sets configured by Registrar/Academic Head staff. They may be assigned as program + term defaults and inherited by sections and delivery groups, with explicit delivery-group overrides when needed. Once a Delivery Pattern version is used by published schedules or enrollments, edits require cloning a new version rather than mutating history.

**Section Delivery Group Logic**: One section may contain students with different delivery setups if they take the same subject set. The default rule is same section + same subject + different delivery groups use the same Registrar-confirmed faculty assignment; the time/modality may differ. If the same faculty assignment is impossible because of workload, availability, eligibility, or conflicts, the row remains blocked until the Registrar changes the approved assignment or section/delivery-group input and reruns readiness.
...
**Sectioning Baseline Rule**: During sectioning, the system lists compatible delivery groups by matching subject set, declared modality, and available capacity. The Registrar confirms the final assignment. The system must not silently auto-section students, auto-balance students, or auto-create overflow sections. Any ranked recommendation or schedule-fit suggestion remains a reviewed enhancement until explicitly approved.

**Online Link Boundary**: TALA records schedule modality, day, time, instructor, section, subject, and room when applicable. TALA does **not** require, validate, store, warn about, or export Zoom, Google Meet, LMS, or platform meeting URLs. Online link coordination remains outside the active system baseline.

**Term Readiness Gate**: The Registrar must first configure the target academic term with `term_name`, `term_start_date`, `term_end_date`, and `scheduling_starts_at`. If any required field is missing, faculty availability submission, availability locking, draft generation, and schedule publication are disabled. The system shows a setup warning listing missing fields (for example: "Schedule setup locked: missing `scheduling_starts_at`").
...
**Pre-Solver Section and Delivery-Group Planning Rule**: TALA must plan sections and section delivery groups before calling the scheduler. The solver does not decide how many sections or delivery groups exist, does not split a year level into new sections, does not create section records during solving, and does not choose teaching subjects for faculty. Registrar/setup staff first create term-scoped sections for each College program and year level, set `curriculum_id`, `year_level`, `curriculum_period`, total section capacity, and one or more delivery groups with modality, delivery pattern, group capacity, and room when required. Only after those records exist does Laravel derive subject demand from `ready_for_scheduling` curriculum scopes. Registrar/setup staff then confirm the subject/faculty assignment inputs from curriculum demand and approved faculty records before CP-SAT places room, day, and time.

**Correct Scheduling Sequence**: After term and curriculum readiness, projected-demand section/delivery-group planning and room inventory may proceed in parallel with faculty availability collection. Registrar-owned eligible subject/faculty assignment follows once sections and availability are ready; room/day/time solving follows only after all hard inputs pass readiness. Faculty availability is the only faculty-owned scheduling input. It does not determine section count or let faculty choose official teaching subjects. A section can be scheduled only when it has explicit year-level/curriculum-period scope, a valid curriculum subject set, and Registrar-confirmed eligible subject/faculty assignment inputs.
...
| Section planning | Defines the academic class block for the target term. | Registrar/setup staff create sections before solving. The solver schedules existing sections and delivery groups only; automatic section creation/splitting is outside the active baseline. |
...
| Faculty teaching assignment | Defines which approved faculty member is assigned to each curriculum-derived subject demand before solving. | Registrar/setup staff confirm subject/faculty assignment inputs from approved staff records; faculty cannot self-select official teaching subjects. |
...
**Faculty Teaching Assignment Rule**: The faculty account creator, Academic Head, Registrar, or another approved administrative owner maintains the approved faculty records used for subject/faculty assignment. Faculty may view their official teaching assignments where exposed, but may not add, remove, replace, or self-approve teaching subjects from their own account. Any change request to teaching assignment is handled outside the faculty self-service flow unless a later typed request workflow is approved.

**Accuracy and Validity Target**: The automatic scheduler target is **greater than 98% auto-assignment coverage for feasible inputs**. Every published official schedule must have **100% hard-constraint validity**. Any generated row with a hard conflict remains a draft conflict and cannot be published.

**MVP Coverage Proof Boundary**: For Phase 1 closure, a feasible-input proof must use a deterministic solver fixture with at least 100 section-subject demands, pre-created sections, fixed rooms for room-required modalities, Registrar-confirmed subject/faculty assignment inputs, submitted/locked availability windows, and avoidable existing commitments. Passing proof means greater than 98% of feasible demands are auto-assigned, unresolved demand remains non-committable, and every assigned row satisfies section, faculty, room, assignment, availability, fixed-room, time, and existing-commitment hard constraints before any official schedule can be committed.

**Section Planning Readiness Flow**:

1. Registrar selects the target term and confirms term readiness.
2. Registrar/setup staff create the planned sections per program and year level, such as `BSIT 1A` or `BSIT 1B`.
...
7. Registrar/setup staff confirm which approved faculty member is assigned to each derived subject demand.
8. Faculty availability defines when assigned faculty may teach.
9. The solver places each section-delivery-group-subject demand into a room when required, day, and time that respects the confirmed faculty assignment and availability input.

**MVP Boundary**: Automatic section creation, automatic delivery-group creation, automatic student balancing across sections, and automatic generation of overflow sections/groups are not active scheduler promises. The simplest active baseline is staff-created sections and delivery groups with capacity enforcement. If a program/year level or delivery setup needs more capacity, Registrar/setup staff create or adjust the section/delivery group before generation and rerun readiness. A lighter assisted sectioning suggestion flow may be reviewed later, but it is not part of the active scheduling baseline until approved.
...
**Implementation Scope**: The scheduling workflow includes Faculty Availability service/UI needed for automatic scheduling: Registrar opens a term availability period, Faculty submits weekly windows during the open period, Registrar reviews/locks submissions, and locked availability feeds the solver snapshot. Post-lock/deadline faculty availability change requests are handled as a controlled exception workflow. Faculty must not directly edit locked availability. Any late or post-lock revision must be filed with a reason, approved or rejected by the Registrar, audited, and captured only in a future generation/superseding-version snapshot.

**Schedule Draft and Publish Surface Boundary**: Schedule generation run records are created by the scheduling service/action layer after term readiness and conflict checks. The Registrar may view runs and review draft rows. The user-visible lifecycle is `Generated Draft` -> `Registrar Review` -> `Registrar Publish`. Publish must call a backend transaction that rejects conflicted, incomplete, stale, or non-feasible runs; creates official `section_meetings`; synchronizes faculty-section-subject assignment; records lifecycle activity; and marks the run published. Academic Head and System Super Admin do not publish schedules. Published schedules are immutable; correction requires a new superseding draft/version and Registrar re-publication. The Admin Nexus must not expose a generic Schedule Draft create/edit form for raw `term_id`, `requested_by`, `constraint_summary`, solver payload, or status mutation.
...
7. If approved after schedule publication, the request must not mutate the published version. Registrar must generate, review, and publish a superseding schedule version if the revision must affect the timetable.
8. Every request, decision, old values, new requested values, actor, timestamp, and reason must be auditable.

**Boundary**: This workflow is not a generic availability editor. Faculty cannot edit `faculty_id`, `term_id`, approval status, lock metadata, or solver snapshot payloads. Registrar cannot bypass validation by typing raw JSON. Approved revisions affect only future generation/reruns and never mutate a published version.

#### 5.3.2 Step 2: Direct Schedule Assignment

- **Manual Assignment Logic**: The Registrar may add or revise a teacher, subject, room, and time as a draft-row fallback. Manual and generated rows use the same review and publication gate; direct creation of an already-official meeting is not allowed.
- **Real-Time Validation (The Commit Guard)**: Upon saving an assignment, the system performs real-time conflict checking:
    ...
    - "Is the teacher an approved assignment for this subject?"
...
**Admin Mapping**: Manual Schedule Assignment is exposed as a Registrar-only add/revise draft-row action under Schedule Drafts. The form uses typed fields for term, section, section delivery group, subject, faculty, room, day, start time, end time, and modality. Staff do not type lifecycle, solver, publication, or audit metadata. Official Schedules are list/view only and are created solely by Registrar publication. Corrections use a new superseding draft/version.

**Conflict Guard**: The guard rejects invalid time ranges, overlapping section assignments, overlapping faculty assignments, overlapping physical-room assignments for on-site/blended meetings, missing Registrar-confirmed faculty assignment, faculty ineligibility, configured workload overflow, and rows outside locked faculty availability windows before publication.

**Faculty-Input Readiness Rule**: Schedule generation is blocked before solver dispatch when any required section-subject demand lacks a Registrar-confirmed eligible faculty assignment, when assigned faculty lacks submitted/locked availability, or when configured workload would be exceeded. Missing availability from non-assigned faculty is not a generation blocker. Manual draft assignment cannot bypass these inputs.

**Manual Assignment Hard-Input Rule**: Manual official-schedule assignment uses the same hard eligibility, submitted-availability, configured-workload, calendar, capacity, faculty, section/delivery-group, and room-conflict guards as automatic generation. Missing or outside availability cannot be bypassed with a reason. The Registrar must correct the assignment or approved input before saving.
...
4. Laravel verifies faculty-input readiness: every required section-subject demand has a Registrar-confirmed faculty assignment and submitted/locked availability evidence for the assigned faculty for the term.
...
10. Laravel validates every returned row against curriculum, section capacity, delivery-group capacity, mandatory Registrar-confirmed faculty assignment, availability, room, section/delivery-group, modality, and conflict rules.
11. Laravel inserts `schedule_draft_rows` with `ok`, `warning`, or `conflict` status.
12. Registrar reviews the run.
13. Registrar selects `Publish Schedule` only for a reviewed run with a feasible solver status and no blocking draft-row conflicts.
14. The publish transaction creates `section_meetings`, updates `section_teacher`, records activity, and marks the run published.
15. Students and faculty can read the schedule only after that Registrar publication succeeds.

**Solver Contract**: The system enforces section planning readiness, delivery-group readiness, faculty-input readiness, solver-status handling, solver-row validation, Registrar publication governance, and 100% validity for every published hard constraint.

**Constraint Coverage Boundary**: The scheduling implementation covers section delivery groups, scoped curriculum readiness, weekly contact hours, delivery patterns, delivery-group capacity, solver statuses, and Registrar publication. Approved subject/faculty eligibility, submitted availability, configured maximum weekly workload, faculty time overlap, section/delivery-group time overlap, room conflict/capacity/type, invalid calendar day/time, missing required delivery group, and missing required curriculum/scope data are hard blockers. Lunch-break windows, max back-to-back load, preference weighting, and any default workload value remain configurable or later optimization inputs until approved with executable tests.

**Post-Publish Immutability Boundary**: Availability or resource changes do not mutate a published schedule. Direct edits and automatic post-publication re-solving are blocked. A correction starts from changed approved inputs, creates a new draft, passes the same hard validation, and supersedes the prior published version only when the Registrar publishes the replacement.
...
| Faculty misses availability deadline                        | Mark as missing/late; generation and manual draft assignment for that faculty remain blocked until an approved submission/revision exists.   | Registrar                         |
| Faculty submits availability late                           | Registrar reviews the revision; once approved it becomes input only for a future generation/superseding version.                              | Registrar                         |
...
| Faculty requests change after schedule publication          | No automatic mutation; approved revision can feed only a new Registrar-reviewed superseding schedule version.                                | Registrar                         |
...
| Export failure after schedule publication                   | Official schedule remains published; export can be retried without changing schedule records                                                 | Registrar                         |
...
#### 5.4.5 Step 5: Enrolled Student Roster Visibility

**Business Rule**: TALA owns the institution's College enrollment lifecycle. Once finance, required physical documents, and section placement are complete, the enrollment becomes `Enrolled`, the account activates, and COR/class access becomes available. Roster viewing or export does not create another TALA enrollment state.
...
2. Registrar may filter the roster by program, year level, section, delivery modality, and student type, then export the filtered data when authorized.
3. The roster includes only student identity, LRN where applicable as prior-education evidence, demographic, guardian, prior-school, program, year level, section, and term fields already held by TALA.
4. Exporting or viewing a roster never changes enrollment state.
5. If staff later discover a conflict that invalidates the institutional enrollment, they use the normal audited enrollment correction/cancellation workflow with a typed reason.
...
**Trigger**: COR availability follows official enrollment handover. It is not a separate reporting state.
...
2. **Contents**: The COR displays student identity needed for institutional use, term, program, section/delivery group, schedule, subject/load details, and authorized assessment/payment-clearance summary fields approved for COR display.
...
8. **Roster Independence**: Roster viewing/export does not affect COR generation. Any later institutionally disqualifying discrepancy uses the normal audited enrollment correction/cancellation workflow.

**Scope Constraint**:

- COR is the immediate core generated academic document because it proves official enrollment.
- SOA/payment acknowledgement artifacts are Accounting-owned internal evidence, not official tax receipts.
- Formal TOR, Form 137, report-card PDF, diploma, certificate, and full credential issuance/fulfillment are outside active TALA scope. Their source records may exist as admission evidence, finalized grade history, or completion status, and students may still view approved/finalized grade records where Student Hub grade visibility is approved. TALA does not generate, request, fulfill, or release those formal credentials.
...
**Ledger Admin Surface Boundary**: The Ledger Entries screen is an Accounting review/evidence surface. Accounting may view and filter assessed fees, payments, discounts, penalties, credits, and balances, but must not create, edit, or delete arbitrary ledger rows through generic Filament CRUD. Core ledger mutations are produced by assessment, discount, payment-confirmation, webhook, or approved adjustment services; installment penalties and refunds remain review scope.
...
- **Trigger**: During the auto-assessment, the system evaluates the student's type and College year level. If `student_type == 'New'` AND `year_level == '1st Year'`, the system calculates 50% of the assessed Tuition Fee.
...
**Mode C: Promissory Note Review Boundary**

Promissory notes are not payment methods and are not active finance-clearance scope for the core sprint. If promoted later, they may record Accounting-reviewed promise/expiry evidence only. They must never clear balance, satisfy minimum downpayment, move a student to `Enrolled`, unlock COR/class-list visibility, or grant exam access by themselves.

#### 6.2.2 Service Fees & Assessments

- **Drop/Withdrawal Fee Boundary**: TALA must not hard-code a flat drop fee. Any automated withdrawal/refund fee assessment remains review scope until the institution approves an effective-dated policy, regulator boundary, role authority, audit trail, and tests. Students may drop even with outstanding balances; any record-release hold must follow an approved policy.
...
- **Scope Contract**: Fee templates use canonical College scope fields: optional program and optional year level. Year-level values must match the enrollment values used by finance services (for example `1st Year`, `2nd Year`, `3rd Year`, `4th Year`). Blank year level means all year levels for the selected College program scope. Only one active fee template may exist for the same program/year scope; older alternatives must be inactive historical records before a replacement becomes active.
...
- **State Transition**: Reaching the minimum downpayment/full-payment threshold creates finance clearance and secures the approved capacity reservation. Applicant-origin enrollment proceeds to **Enrolled** only when admission gates and placement also succeed. Retention undertakings remain separate holds. Promissory notes do not trigger these transitions.
...
**Installment Policy Review Boundary**:

Installment plans, penalties, grace periods, and per-student installment-state automation remain review scope. The active core sprint uses assessment, confirmed payments, immutable ledger entries, and minimum-downpayment/full-payment finance clearance.

**Exam Access Review Boundary**:

TALA must not create an undocumented debt-based exam block. Digital exam permits and exam-access accommodation workflows remain review scope. Promissory-note approval must not grant exam access by itself. Any future exam-access feature must hide balances, certification files, certification numbers, payment channels, and promissory amounts from Faculty, public verification, and ordinary student-facing decision responses.

#### 6.2.4 Financial Disposition Review Boundary

Payment cancellation, refund, withdrawal-fee, and financial-disposition automation remains review scope. The active core sprint preserves immutable payment/ledger evidence and blocks raw ledger edits. Any future disposition workflow must be versioned, effective-dated, institution-approved, regulator-bounded, role-authorized, auditable, and tested.
...
1.  **Within 15 Days of Payment Reference**: An authorized refund request for the admission/enrollment fee is evaluated against the effective policy, payment channel, externally issued receipt/reference evidence, prior dispositions, and official-enrollment evidence. Approval and payout mechanics require finance reconciliation and immutable audit evidence.
...
2.  **Credit Application**: This negative balance acts as a credit. When a new approved fee is assessed, such as next semester's tuition, it adds to the ledger and offsets against the negative balance.
...
- **Document Requirements — College**: Required documents for new freshmen, transferees, returnees, and approved College applicant pathways.
...
- Clicking "Copy College Requirements Link" copies `tala.edu.ph/admission-requirements#college` to the clipboard.
...
**Term Rule**: When opened from a class list, the advising modal uses the viewed class/section term. If no viewed term is available, it uses the configured active College term. If neither exists, the modal shows **Not Available** instead of using the latest historical term.
...
- **Year Level**: Current College year level and program.
...
**Efficiency Strategy**: To balance simplicity and accuracy, the system uses **Period-Level Entry** for College grade profiles. Faculty enters the required period scores/grades defined by the active College grading profile; TALA stores the resulting period and final-grade evidence.

#### 7.2.1 Step 1: Program-Specific Grading Logic

The system applies the active College grading calculation profile for the student's term, program, subject, and delivery scope.

**College (Profile-Based)**:
...
4. **Finalize**: Registrar verification atomically finalizes the verified package into official academic history. Only verified/finalized grades are released to the Student Hub and prerequisite/progression services. Formal transcript, report-card PDF, and full credential generation remain outside active TALA scope, but grade records and grade viewing remain active.
...
- **Optional Filters**: Program, year level, section, delivery modality, and student type.
- **Columns/Export Fields**: Student ID, LRN where applicable as prior-education evidence, complete name, demographics required by the institution, guardian details, prior school, program, year level, section, modality, and term.
...
- **Template Boundary**: MVP provides one generic TALA roster schema in both CSV and XLSX. Regulator-specific templates are not generated; staff may use the generic export as authorized source data.
...
- **Export Boundary**: Staff decide how and when to use exported data outside TALA. TALA neither tracks nor asserts completion outside the system.
...
| **Academic Head**      | **Read-Only** scheduling oversight; authorized approval only for separately defined academic actions such as finalized-grade correction |
...
| **Curriculum**     | Manage College programs and courses (Add/Edit dynamic options)            |
...
| Module 2 | Schedule Published                          | "Your class schedule is available"                                  | Registrar publishes the reviewed official schedule                                        |

---

### 8.7 Frequently Asked Questions & Support

The system includes an FAQ/help content module accessible via the public landing page and Student Hub. FAQ entries are maintained through a permission-gated Admin Nexus CRUD surface so support content is not hardcoded. Public users and students can read published guidance before using the applicable module workflow or outside-office procedure. This feature reduces support load while ensuring consistent guidance.
...
- **Step 4 (Escalation)**: If the answer is not in FAQ, the user proceeds to the applicable module workflow or outside-office procedure; FAQ does not create a generic ticket or document-request workflow.
...
### Appendix A: Current Transitional College Grading Profile (Pending Institution Approval)
...
### Appendix B: College Document Requirements Matrix

This appendix lists active College admission evidence. Grade 12/Form 138/Form 137 records appear only as prior-education evidence for College applicants.

| Document              | New Student | Transferee |
| --------------------- | ----------- | ---------- |
| PSA Birth Certificate | ✅          | ✅         |
| Grade 12 Report Card / Form 138 prior-education evidence | ✅ | As applicable |
| Form 137              | ✅          | ✅         |
...
### Appendix D: Factual Alignment Addendum (2026-05-22)
...
1. **College Curriculum Structures**
   - College subjects are quantified using standard **Units** (Lec, Lab, Credit). Non-College curriculum structures are outside the active product baseline.
...
2. **Standard Fee Terminology**
   - All enrollment billing, SOAs, and Accounting views MUST use the following exact fee categories as default seeded line items:
     ...
     - `Tuition Fee` (College value varies by effective fee template)
     - `Other Fees` (College value varies by effective fee template)
