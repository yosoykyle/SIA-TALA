...
**Current Backend Contract Note (SDD-05B)**: `StudentEnrollmentService` now owns approved-applicant enrollment creation, regular enrollment start, returnee detection, section/delivery-group capacity assignment, finance-cleared account handover, and COR readiness checks. Manual Accounting payment confirmation delegates handover to this service so `Pre-Enrolled`, active account status, Student ID username, student role assignment, and COR readiness evidence are committed together for the manual payment path. Online gateway payment parity is tracked in the SDD-06 payment slice.
