...
| TAL-93 | In progress - only TAL-93J3c remains | Cross-Role Regression, Security, and UAT Readiness (Pre-Integration Gate). TAL-93A-I, J1, J2, J2a-h, J3a, and J3b are done locally; the final J3c gate must pass before integration hardening (TAL-94/95) begins. See the TAL-93 Sub-Slice Map below. |
...
| TAL-102 | Planned (future enhancement, post-MVP) | Preferred-Time Scheduling Optimization: optional `PREFERRED` faculty calendar-event capture, solver scoring/contract changes, benchmarked objective weighting, explainable candidate comparison, and focused validation. Routed from PRD Modules 04–06 and the architecture availability boundary. V1 enforces unavailable blocks only; this is not an MVP or TAL-94 dependency. |

### Conditional Proposals Without Active Tasks

The following are not approved work and must not be inferred as upcoming slices: program-specific or graduate-school calendar variants; component types beyond Lecture/Laboratory; optional charts, timetable visualization, saved-filter or UI plugins; retained server-generated PDFs; public QR/artifact verification; and Redis/Horizon deployment scaling. Each remains excluded from the active sequence. A future institutional requirement must pass the protocol gates and receive a new approved Next Steps issue before implementation.
...
| TAL-93J3b | Docs-only authority correction | Clarified TALA's companion/integration boundary and CP-SAT central contribution; classified Module 13 capability depth; reconciled active deferral and upcoming-work notes against this tracker. No code, test, schema, DB, or historical research change. | Done locally; pending Linear sync |
| TAL-93J3c | Final gate | Verify all five staff roles against every registered staff surface; run the complete automated regression, static-analysis, dependency-security, build, and thin rendered-smoke stack; produce only the visual, institutional-workflow, real-email, and later-integration checklist that still requires people. Pure pass/fail; no product fixes. | Awaiting plan |

### Next Boundary

Next primary boundary: **Plan TAL-93J3c** (pure final pre-integration gate). J3b found no implementation mismatch requiring a routed fix. Once J3c is complete, remove the entire TAL-93 parent block from this file and advance to TAL-94 planning.
