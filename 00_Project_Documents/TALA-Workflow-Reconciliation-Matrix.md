...
- Document-request catalog/fulfillment and enrollment adjustment workflows are `UAT_SUPPORTING` unless they appear in the chosen presentation path.
- Refund lifecycle, courier automation, advanced analytics, and public FAQ polish are `UAT_DEFERRED` unless the user explicitly promotes them. External DepEd/CHED/LIS portal encoding is `UAT_EXTERNAL`; the generic school-year/term enrolled-student inventory remains part of the TALA core/supporting baseline.
...
| Official documents | Section C / SDD-07B and SDD-08A | Group 7 lock-audited; COR controls partially implemented and core, full issued-document/PDF/release lifecycle supporting/deferred |
...
| --- | --- | --- | --- | --- | --- |
| College calendars, enrollment windows, adjustment windows, exams, and term close | FS 5.1.1, 10; TS 3.18 | `AcademicYear`, `Term`, `CalendarPhaseGateService`; calendar tests currently contain education-level branching | `RECONCILE` | Remove active SHS calendar branches and keep College term/phase gates. Academic Head approval and minimum-day evidence remain a later admin contract. | SDD-00C / SDD-04 / SDD-09 |
...
| Deprecated SHS progression/remediation | Historical workflow evidence only | SHS-related services/tests may still exist in runtime | `RECONCILE` | Remove SHS progression, remediation, grade-level promotion, and Grade 11/12 enrollment behavior from active specs, UI, services, seeders, and tests. Preserve archived evidence only for history. | SDD-00C |
...
| Official document issuance snapshots, private artifacts, and public verification | FS 5.6/9.1; TS 2.6.2/3.14.2.1; BM-13/BM-14/BM-15/BM-22/BM-23 | COR verification token lifecycle exists; document request lifecycle exists; no full issuance table/service/PDF/source snapshot/checksum/public verification route | `RECONCILE` | Build one issuance service boundary. COR is UAT-core after canonical enrollment. Other academic documents, SOA/receipts, and diploma/certificate artifacts are supporting/deferred by owning workflow. Generated files are private derived artifacts with source snapshot/checksum/template/reference/release state. QR/public verification uses opaque/signed minimal disclosure only. | SDD-07B / SDD-08A / SDD-06E / SDD-07E |
...
| Deprecated SHS faculty grading workflow | Historical workflow evidence only | `SHSGradingService`, SHS grade tests, and SHS UI actions may still exist in runtime | `RECONCILE` | Remove active SHS grading services, UI options, seed data, and tests during the College-only cleanup. Archived SHS grade evidence must not define active grade behavior. | SDD-00C |
...
| Student Hub read-only visibility and PWA cache boundary | FS 2.3/4.3; TS 1.4/5.6.2/5.8; BM-12/BM-24 | `/student/*` routes, `EnsureActiveStudentHubUser`, Student layout PWA directives, published Help/FAQ, `StudentDashboardService`, and access/dashboard tests exist. Dashboard, Schedule, Grades, Financials, and Documents are still placeholder/static; `public/sw.js` is generic offline fallback only. | `RECONCILE` | Retain the active-student, owner-scoped service contract as the authority. Connect Student Hub pages to `StudentDashboardService`, remove sample values, keep student mutations disabled/deferred until services/forms exist, and do not cache protected finance/grade/COR data until freshness labels, cache versioning, logout/account-denial clearing, and offline mutation denial are tested. | SDD-08B / TAL-13 |
...
10. Generated official documents are derived artifacts, not primary operational records. COR, SOA, receipts, TOR/Form 137 copies, report cards, and diplomas/certificates must be generated from authoritative enrollment, ledger, grade, student-record, and document-request data with template version, issuer, checksum, serial/reference where applicable, and revoke/supersede behavior. QR verification must resolve through opaque/signed routes or tokens rather than embedding private student data.
11. Registrar enrollment-verification/transcript benchmarks distinguish enrollment certifications from permanent academic records and place release authority with the Registrar/student-record office. DepEd school-record guidance treats Form 137/SF10 and Form 138/SF9 as confidential learner records with controlled request/release processes. TALA therefore records request, release, and verification evidence but does not become DepEd LIS or a receiving-school portal.
...
3. **SDD-05C-R and SDD-08A policy closure:** College progression plus versioned College grading profiles. Do not change historical or current grades without an approved profile and migration rule.
