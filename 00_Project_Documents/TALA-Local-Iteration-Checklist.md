...
- [x] Split the newly surfaced workflow coverage into SDD-07B document fulfillment, SDD-07C enrollment adjustments, SDD-07D record/status lifecycle, and SDD-07E graduation evaluation instead of treating generic `service_requests` resolution as implementation.
...
- [x] Lock initial intake exposure: publish Regular SHS, SHS Transfer, Regular College Freshman, and College Transfer; keep cross-enrollee inactive; represent old curriculum/ALS, foreign compliance, and IP/SEN support through composable rules rather than separate pipelines.
...
- [x] Approve the Regular SHS requirement contract: admission requires verified identity plus verified Grade 10 completion/eligibility through a recorded regulator-permitted method; Good Moral, 2x2 photo, Certificate of Completion, and Form 137 are configurable retention/follow-up items rather than universal initial blockers.
...
- [x] Approve the SHS Transfer requirement contract: admission requires verified identity plus latest-completed-grade eligibility; Good Moral, one ID-photo obligation, official transfer credentials, and Form 137 are retention/follow-up; the prior-grade requirement resolves from the target grade rather than hardcoding Grade 12.
...
- [x] Benchmark and approve the ALS pathway: current client policy accepts ALS only for applicants taking Grade 11. Model ALS as `als_jhs` under Regular SHS Grade 11, requiring verified identity plus one authoritative ALS Junior High School-level eligibility outcome. Do not publish ALS for College, transfer, Grade 12, or generic applicant intake in the current deployment.
...
- [x] Benchmark and approve tentative-placement expiry, seat-consumption, readiness order, and SHS/College capacity handling.
  ...
  - 2026-06-19 capacity rule: the current institution may seed a top-level 100-active-student campus cap, but SHS/College/program/grade/delivery caps are optional effective-dated sub-plans. OR-backed payment secures capacity against every matching plan; tentative pre-payment placement consumes no protected capacity and expires at payment deadline, admission-window close, or Registrar cancellation.
