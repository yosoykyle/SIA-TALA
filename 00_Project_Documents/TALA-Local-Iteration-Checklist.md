...
- [x] Capture user decisions for `F18`, `F39`, `FIN15`, `INT09`, `INT10`, `INT11`, and `INT12`.
- [x] Update FS/TS boundaries so removed items are not described as future implementation goals.
- [x] Update reconciliation and SDD execution control so `SDD-00E` blocks implementation until active docs/UAT match the new scope.
- [x] Update master UAT rows to remove cross-enrollee and official-receipt issuance test cases, and reword payment references where needed.
- [x] Mirror `SDD-00E` in Linear and mark `TAL-31` blocked until the role/module feature rebaseline is complete.
  - 2026-06-22 Linear mirror: created urgent in-progress `TAL-32` (`SDD-00E - Scope pruning and role-feature rebaseline`) under `TAL-12`, related it to `TAL-31`, and updated/commented on `TAL-31` to show CP-SAT work is paused until SDD-00E closes.
- [x] Define the SDD-00E role/module backlog effect: completed compatible implementation remains evidence; in-progress work can be blocked or reprioritized; backlog rows tied to removed/external features are removed or rewritten as evidence/export boundaries.
...
| Student Hub / Applicant UI | Keep applicant intake UI and protected read-only Student Hub visibility. | TAL-13/SDD-08B activates after backend services are stable; document-request UI stays removed. |
| Imports/exports/reports | Keep curriculum import and generic enrolled roster export. | Roster export may be P0 under SDD-07A; broader reporting stays SDD-09/P1. |
| Student status/completion | Keep truthful boundary and typed future lifecycles only. | SDD-07C/07D/07E stay P1 unless UAT path promotes one. |
| Attendance/discipline/guidance/analytics | Keep no-hidden-blocker invariant; future policy-gated work only. | P2/deferred; do not consume two-day rescue time. |
...
- [x] Advanced shipping automation remains outside rescue; manual Registrar fulfillment and hardened payment/reference evidence may remain.
...
- [ ] Revalidate the SDD-07A payment/capacity sequence against the approved pre-payment section/schedule preparation and payment-secured slot workflow; preserve the external LIS/CHED boundary, generic roster, versioned requirements, and assistive-only OCR where compatible.
...
- [x] Lock initial intake exposure after SDD-00E: publish only College Freshman and College Transfer paths for active deployment; remove SHS and cross-enrollee as active/dormant offerings; keep unapproved ALS/equivalency routes unpublished; represent old curriculum, foreign compliance, and IP/SEN support through composable College rules rather than separate pipelines.
- [x] Lock returning-student boundary: use Registrar-assisted lookup/readmission; when no reliable pre-TALA profile exists, create a provenance-tagged legacy baseline after duplicate review, then continue through SDD-07D readmission rather than public new-applicant creation.
  - 2026-06-19 Linear mirror, superseded by SDD-00E for cross-enrollee: initial offerings, composable profile treatment, inactive cross-enrollee status, legacy onboarding limits, and SDD-07A/07D ownership were posted to `TAL-28`.
...
- [x] Benchmark and approve tentative-placement expiry, seat-consumption, readiness order, and College capacity handling.
  ...
  - 2026-06-19 capacity rule, updated by SDD-00E: the current institution may seed a top-level 100-active-student campus cap, while College program/year/delivery caps are optional effective-dated sub-plans. Confirmed payment/reference evidence secures capacity against every matching plan; tentative pre-payment placement consumes no protected capacity and expires at payment deadline, admission-window close, or Registrar cancellation.
...
- [x] Implement the first SDD-07A Filament admin setup surfaces.
  ...
  - 2026-06-19 code evidence, wording updated by SDD-00E: setup forms expose term-scoped offering dimensions, policy version/effective approval fields, per-requirement gate/evidence/storage/OCR fields, and effective capacity-plan scope/limit fields. `reserved_count` remains read-only on capacity plans and is still updated only by payment-backed finance clearance.
