...
> **Archived implementation history — not current authority.** Canonical 00–06 supersedes this charter's product assumptions and task sequence. No status, plan, contract, or instruction below grants present execution authority.

**Historical status:** Former governing charter for the remaining TAL-96D plans; superseded by the definition-first rebaseline

**Applies to remaining work:** `TAL-96D5E1` through `TAL-96D5E2`

**Next execution boundary:** TAL-96D5E1A through TAL-96D5E1C, TAL-96D5E1D1-D5, and TAL-96D5E1D6A-D6C are independently verified and cleaned locally. B2A delivered Academic Readiness and the combined curriculum encoding/import-review interaction; B2B delivered the Admissions Work Queue and contextual Applicant Record while preserving authoritative records; B2C delivered the Class Planning operating sequence with role-safe contextual source links and blocker-owned remediation routes; B3 delivered Enrollment, Student Record, and Lifecycle recovery with affected-role projections; C delivered the task-centered Accounting flow and PayMongo exception recovery while preserving normalized finance records; D1 closed capability and presentation authority; D2 recovered the timetabling operating journey; D3 closed the Enrollment-to-COR operating journey and source-derived presentation context; D4 closed the Admissions-to-Student journey; D5 closed the student-account payment-to-clearance journey; and D6A-D6C recovered grades, lifecycle, and completion presentation. The approved TAL-96D5E1D6D1 boundary now replaces the current `MIDDLE` database contents with a recoverable, client-aligned `MIN` presentation fixture before D6D2 continues System Administration presentation. Historical MIDDLE acceptance and capacity evidence remains historical and unchanged. Consolidated visual/manual acceptance remains owned by TAL-96D5E1E. No capacity rerun, deployment, production promotion, equation, schema change, live provider request, or unclassified live-surface deletion or merge is implied.
...
4. `TAL-96D5E1D` performs the approved lean-MVP authority and workflow consolidation. It classifies every visible capability as primary, contextual, evidence-only, deferred, or retired; reduces each role to task-centered navigation; vertically recovers the six demonstration-critical cross-role journeys; preserves proven domain services and authoritative records; and closes the shared responsive-table, business-question-filter, status, action, date, empty/blocked/recovery, and contextual-route contracts before D5E1E rendered acceptance.
5. `TAL-96D5E1D6D1` prepares the client-aligned `MIN` database, stable defense accounts, and only the case overlays needed to exercise approved journeys without misrepresenting the current population.
6. `TAL-96D5E1E` uses the frozen `MIN` presentation personas for code-first evidence and a concise role-organized human acceptance pass. It repeats only failed or corrected journeys.
...
For D5E1D, capability inclusion and primary navigation are separate decisions. A capability may remain implemented and directly authorized while being presented contextually or as investigator-only evidence. The canonical disposition register, exact role navigation, and six-journey surface map live in `ui_surface_blueprint.md`; PRD Module 02 owns the role responsibilities and task-centered interaction contract. These authorities prevent a numbered PRD responsibility from being misread as a requirement for another peer sidebar item.
...
- TAL-96D5E1A-D6C preserve the verified MIDDLE-based recovery evidence as historical implementation evidence.
- TAL-96D5E1D6D1 replaces only the current presentation database contents with the client-aligned `MIN` baseline: 47 current students in six first-/second-year sections, nine faculty, six rooms, real curriculum source rows, and the 54-demand scheduling workload. Later curriculum years remain authoritative curriculum content; they do not create an active third-year census.
- TAL-96D5E1D6D2-D7 complete remaining presentation and cross-role convergence against the stable `MIN` fixture.
- TAL-96D5E1E owns email and PayMongo acceptance evidence, developer spin-up, the first-time journey guide, and guided human acceptance over the frozen `MIN` presentation fixture.
...
| TAL-96D5E1 | Client-aligned `MIN` presentation personas and compatible operational states, email and PayMongo acceptance, developer spin-up, first-time role journey guide, and guided exploration/remediation; prior MIDDLE evidence remains historical | No capacity study; at most one separately authorized functional `MIN` solve when no accepted candidate can be restored |
...
| TAL-96D5E1D | Lean-MVP authority and workflow consolidation across all visible capabilities, role navigation, six demonstration-critical journeys, and shared presentation contracts | No schema redesign, source-record merge, solver/provider change, unclassified live-surface deletion, broad visual rewrite, or dependency addition |
| TAL-96D5E1D6D1 | Guarded client-aligned `MIN` presentation-database replacement, Filipino-sounding fictional identities, stable defense accounts, and case-state overlays | No active third-year cohort, schema redesign, live provider request, cost-bearing solve, production database mutation, or real client identity |
| TAL-96D5E1D6D2 | System Administration boundary and health presentation after the presentation fixture is stable | No fixture replacement or unrelated role expansion |
| TAL-96D5E1E | Exploration evidence, first-time guide, and concise human acceptance over the frozen `MIN` presentation fixture | No capacity study; repeat only failed or corrected representative journeys |
